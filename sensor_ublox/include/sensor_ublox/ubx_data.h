...
Poll GPS Aiding Data (Almanac) for all 32 SVs by sending this message to the receiver without any payload. The receiver
will return 32 messages of type AID-ALM as defined below.
...
Poll GPS Aiding Data (Almanac) for an SV by sending this message to the receiver. The receiver will return one message
of type AID-ALM as defined below.
...
* If the WEEK Value is 0, DWRD0 to DWRD7 are not sent as the Almanac is not available for the given SV. This may happen
even if NAV-SVINFO and RXM-SVSI are indicating almanac availability as the internal data may not represent the content
of an original broadcast almanac (or only parts thereof).
* DWORD0 to DWORD7 contain the 8 words following the Hand-Over Word ( HOW ) from the GPS navigation message, either
pages 1 to 24 of sub-frame 5 or pages 2 to 10 of subframe 4. See IS-GPS-200 for a full description of the contents of
the Almanac pages.
* In DWORD0 to DWORD7, the parity bits have been removed, and the 24 bits of data are located in Bits 0 to 23. Bits 24
to 31 shall be ignored.
* Example: Parameter e (Eccentricity) from Almanac Subframe 4/5, Word 3, Bits 69-84 within the subframe can be found in
DWRD0, Bits 15-0 whereas Bit 0 is the LSB.
...
Poll <i>AssistNow Autonomous</i> aiding data for all GPS satellites by sending this empty message. The receiver will
return an AID-AOP message (see definition below) for each GPS satellite for which data is available.
...
Poll the <i>AssistNow Autonomous</i> data for the specified GPS satellite. The receiver will return a AID-AOP message
(see definition below) if data is available for the requested satellite.
...
If enabled, this message is output at irregular intervals. It is output whenever <i>AssistNow Autonomous</i> has
produced new data for a satellite. Depending on the availability of the optional data the receiver will output either
version of the message. If this message is polled using one of the two poll requests described above the receiver will
send this message if AssistNow Autonomous data is available or the corresponding poll request message if no AssistNow
Autonomous data is available for each satellite (i.e. svid 1..32). At the user's choice the optional data may be chopped
from the payload of a previously polled message when sending the message back to the receiver. Sending a valid AID-AOP
message to the receiver will automatically enable the <i>AssistNow Autonomous</i> feature on the receiver. See the
section <r href=\"AOP-DESC'>AssistNow Autonomous</r> in the receiver description for details on this feature.
...
Poll GPS Aiding Data (Ephemeris) for all 32 SVs by sending this message to the receiver without any payload. The
receiver will return 32 messages of type AID-EPH as defined below.
...
Poll GPS Constellation Data (Ephemeris) for an SV by sending this message to the receiver. The receiver will return one
message of type AID-EPH as defined below.
...
* SF1D0 to SF3D7 is only sent if ephemeris is available for this SV. If not, the payload may be reduced to 8 Bytes, or
all bytes are set to zero, indicating that this SV Number does not have valid ephemeris for the moment. This may happen
even if NAV-SVINFO and RXM-SVSI are indicating ephemeris availability as the internal data may not represent the content
of an original broadcast ephemeris (or only parts thereof).
* SF1D0 to SF3D7 contain the 24 words following the Hand-Over Word ( HOW ) from the GPS navigation message, subframes 1
to 3. The Truncated TOW Count is not valid and cannot be used. See IS-GPS-200 for a full description of the contents of
the Subframes.
* In SF1D0 to SF3D7, the parity bits have been removed, and the 24 bits of data are located in Bits 0 to 23. Bits 24 to
31 shall be ignored.
* When polled, the data contained in this message does not represent the full original ephemeris broadcast. Some fields
that are irrelevant to u-blox receivers may be missing. The week number in Subframe 1 has already been modified to match
...
This message contains a health bit mask, UTC time and Klobuchar parameters. For more information on these parameters,
see the ICD-GPS-200 documentation.
...
#define UBX_AID_HUI_DATA1_FLAGS_HEALTHVALID_GET(val) \
    (U)(((val)&UBX_AID_HUI_DATA1_FLAGS_HEALTHVALID_MASK) >> 0) //!< Get healthValid from bitmask flags
#define UBX_AID_HUI_DATA1_FLAGS_UTCVALID_MASK 0x2              //!< Mask for field utcValid in bitmask flags
#define UBX_AID_HUI_DATA1_FLAGS_UTCVALID_GET(val) \
    (U)(((val)&UBX_AID_HUI_DATA1_FLAGS_UTCVALID_MASK) >> 1) //!< Get utcValid from bitmask flags
#define UBX_AID_HUI_DATA1_FLAGS_KLOBVALID_MASK 0x4          //!< Mask for field klobValid in bitmask flags
#define UBX_AID_HUI_DATA1_FLAGS_KLOBVALID_GET(val) \
    (U)(((val)&UBX_AID_HUI_DATA1_FLAGS_KLOBVALID_MASK) >> 2) //!< Get klobValid from bitmask flags
...
This message contains position, time and clock drift information. The position can be input in either the ECEF X/Y/Z
coordinate system or as lat/lon/height. The time can either be input as inexact value via the standard communication
interface, suffering from latency depending on the baud rate, or using hardware time synchronization where an accurate
time pulse is input on the external interrupts. It is also possible to supply hardware frequency aiding by connecting a
continuous signal to an external interrupt.
...
#define UBX_AID_INI_U6_TMCFG_FEDGE_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_TMCFG_FEDGE_MASK) >> 1) //!< Get fEdge from bitmask tmCfg
#define UBX_AID_INI_U6_TMCFG_TM1_MASK 0x10            //!< Mask for field tm1 in bitmask tmCfg
#define UBX_AID_INI_U6_TMCFG_TM1_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_TMCFG_TM1_MASK) >> 4) //!< Get tm1 from bitmask tmCfg
...
#define UBX_AID_INI_U6_FLAGS_POS_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_POS_MASK) >> 0) //!< Get pos from bitmask flags
#define UBX_AID_INI_U6_FLAGS_TIME_MASK 0x02         //!< Mask for field time in bitmask flags
#define UBX_AID_INI_U6_FLAGS_TIME_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_TIME_MASK) >> 1) //!< Get time from bitmask flags
#define UBX_AID_INI_U6_FLAGS_CLOCKD_MASK 0x04        //!< Mask for field clockD in bitmask flags
#define UBX_AID_INI_U6_FLAGS_CLOCKD_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_CLOCKD_MASK) >> 2) //!< Get clockD from bitmask flags
...
#define UBX_AID_INI_U6_FLAGS_CLOCKF_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_CLOCKF_MASK) >> 4) //!< Get clockF from bitmask flags
#define UBX_AID_INI_U6_FLAGS_LLA_MASK 0x20             //!< Mask for field lla in bitmask flags
#define UBX_AID_INI_U6_FLAGS_LLA_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_LLA_MASK) >> 5) //!< Get lla from bitmask flags
#define UBX_AID_INI_U6_FLAGS_ALTINV_MASK 0x40       //!< Mask for field altInv in bitmask flags
#define UBX_AID_INI_U6_FLAGS_ALTINV_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_ALTINV_MASK) >> 6) //!< Get altInv from bitmask flags
#define UBX_AID_INI_U6_FLAGS_PREVTM_MASK 0x80          //!< Mask for field prevTm in bitmask flags
#define UBX_AID_INI_U6_FLAGS_PREVTM_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_PREVTM_MASK) >> 7) //!< Get prevTm from bitmask flags
#define UBX_AID_INI_U6_FLAGS_UTC_MASK 0x400            //!< Mask for field utc in bitmask flags
#define UBX_AID_INI_U6_FLAGS_UTC_GET(val) \
    (U)(((val)&UBX_AID_INI_U6_FLAGS_UTC_MASK) >> 10) //!< Get utc from bitmask flags
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-ANT
with a payload as defined below
...
#define UBX_CFG_ANT_DATA0_FLAGS_SVCS_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_FLAGS_SVCS_MASK) >> 0) //!< Get svcs from bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_SCD_MASK 0x0002         //!< Mask for field scd in bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_SCD_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_FLAGS_SCD_MASK) >> 1) //!< Get scd from bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_OCD_MASK 0x0004        //!< Mask for field ocd in bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_OCD_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_FLAGS_OCD_MASK) >> 2) //!< Get ocd from bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_PDWNONSCD_MASK 0x0008  //!< Mask for field pdwnOnSCD in bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_PDWNONSCD_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_FLAGS_PDWNONSCD_MASK) >> 3) //!< Get pdwnOnSCD from bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_RECOVERY_MASK 0x0010         //!< Mask for field recovery in bitmask flags
#define UBX_CFG_ANT_DATA0_FLAGS_RECOVERY_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_FLAGS_RECOVERY_MASK) >> 4) //!< Get recovery from bitmask flags
...
#define UBX_CFG_ANT_DATA0_PINS_PINSWITCH_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_PINS_PINSWITCH_MASK) >> 0) //!< Get pinSwitch from bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_PINSCD_MASK 0x03E0           //!< Mask for field pinSCD in bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_PINSCD_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_PINS_PINSCD_MASK) >> 5) //!< Get pinSCD from bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_PINOCD_MASK 0x7C00        //!< Mask for field pinOCD in bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_PINOCD_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_PINS_PINOCD_MASK) >> 10) //!< Get pinOCD from bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_RECONFIG_MASK 0x8000       //!< Mask for field reconfig in bitmask pins
#define UBX_CFG_ANT_DATA0_PINS_RECONFIG_GET(val) \
    (U)(((val)&UBX_CFG_ANT_DATA0_PINS_RECONFIG_MASK) >> 15) //!< Get reconfig from bitmask pins
...
See <r href=\"CFG-CFG-DESC'>Receiver Configuration</r> for a detailed description on how Receiver Configuration should
be used. The three masks are made up of individual bits, each bit indicating the sub-section of all configurations on
which the corresponding action shall be carried out. The reserved bits in the masks must be set to '0'. For detailed
information refer to the <r href=\"orgConfigSect'>Organization of the Configuration Sections</r>. Note that commands can
be combined. The sequence of execution is Clear, Save, Load
...
typedef struct UBX_CFG_CFG_U5_s {
    X4 clearMask; //!< Mask with configuration sub-sections to clear (i.e. load default configurations to permanent
                  //! configurations in non-volatile memory)
    X4 saveMask;  //!< Mask with configuration sub-sections to save (i.e. save current configurations to non-volatile
                  //! memory), see ID description of clearMask
    X4 loadMask; //!< Mask with configuration sub-sections to load (i.e. load permanent configurations from non-volatile
                 //! memory to current configurations), see ID description of clearMask
...
#define UBX_CFG_CFG_U5_CLEARMASK_IOPORT_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_IOPORT_MASK) >> 0) //!< Get ioPort from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_MSGCONF_MASK 0x0002       //!< Mask for field msgConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_MSGCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_MSGCONF_MASK) >> 1) //!< Get msgConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_INFMSG_MASK 0x0004         //!< Mask for field infMsg in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_INFMSG_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_INFMSG_MASK) >> 2) //!< Get infMsg from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_NAVCONF_MASK 0x0008       //!< Mask for field navConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_NAVCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_NAVCONF_MASK) >> 3) //!< Get navConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_RXMCONF_MASK 0x0010        //!< Mask for field rxmConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_RXMCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_RXMCONF_MASK) >> 4) //!< Get rxmConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_RINVCONF_MASK 0x0200       //!< Mask for field rinvConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_RINVCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_RINVCONF_MASK) >> 9) //!< Get rinvConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_ANTCONF_MASK 0x0400         //!< Mask for field antConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_ANTCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_ANTCONF_MASK) >> 10) //!< Get antConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_LOGCONF_MASK 0x0800         //!< Mask for field logConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_LOGCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_LOGCONF_MASK) >> 11) //!< Get logConf from bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_FTSCONF_MASK 0x1000         //!< Mask for field ftsConf in bitmask clearMask
#define UBX_CFG_CFG_U5_CLEARMASK_FTSCONF_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_CLEARMASK_FTSCONF_MASK) >> 12) //!< Get ftsConf from bitmask clearMask
...
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVBBR_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVBBR_MASK) >> 0) //!< Get devBBR from bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVFLASH_MASK 0x02 //!< Mask for field devFlash in bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVFLASH_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVFLASH_MASK) >> 1) //!< Get devFlash from bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVEEPROM_MASK 0x04 //!< Mask for field devEEPROM in bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVEEPROM_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVEEPROM_MASK) >> 2) //!< Get devEEPROM from bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVSPIFLASH_MASK 0x10 //!< Mask for field devSpiFlash in bitmask deviceMask
#define UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVSPIFLASH_GET(val) \
    (U)(((val)&UBX_CFG_CFG_U5_DEVICEMASK_DEVICEMASK_DEVSPIFLASH_MASK) >> 4) //!< Get devSpiFlash from bitmask deviceMask
...
Returns the parameters of the currently defined datum. If no user-defined datum has been set, this will default to
WGS84.
...
/*!
Poll DOSC settings
Sending this message to the receiver results in the receiver returning a message of type CFG-DOSC-DATA0 with a payload
as defined below for the oscillator specified
...
The gainVco and gainUncertainty parameters are normally set using the <r href=\"FTS-calib'>calibration process</r>
initiated using <rc href=\"UBX-TIM-VCOCAL'>UBX-TIM-VCOCAL</rc>.
The behavior of the system can be badly affected by setting the wrong values, so customers are advised to only change
...
typedef struct UBX_CFG_DOSC_DATA0_OSCID_s {
    ...
    U2 timeToTemp; //!< The minimum time that it could take for a temperature variation to move the oscillator frequency
                   //! by 'withTemp' (must be > 0)
...
#define UBX_CFG_DOSC_DATA0_OSCID_FLAGS_ISCALIBRATED_GET(val) \
    (U)(((val)&UBX_CFG_DOSC_DATA0_OSCID_FLAGS_ISCALIBRATED_MASK) >> 0) //!< Get isCalibrated from bitmask flags
#define UBX_CFG_DOSC_DATA0_OSCID_FLAGS_CONTROLIF_MASK 0x001E           //!< Mask for field controlIf in bitmask flags
#define UBX_CFG_DOSC_DATA0_OSCID_FLAGS_CONTROLIF_GET(val) \
    (U)(((val)&UBX_CFG_DOSC_DATA0_OSCID_FLAGS_CONTROLIF_MASK) >> 1) //!< Get controlIf from bitmask flags
...
External time or frequency source configuration. The stability of time and frequency sources is described using
different fields, see sourceType field documentation.
...
#define UBX_CFG_ESRC_DATA0_EXTINT_FLAGS_POLARITY_GET(val) \
    (U)(((val)&UBX_CFG_ESRC_DATA0_EXTINT_FLAGS_POLARITY_MASK) >> 0) //!< Get polarity from bitmask flags
#define UBX_CFG_ESRC_DATA0_EXTINT_FLAGS_GNSSUTC_MASK 0x02           //!< Mask for field gnssUtc in bitmask flags
#define UBX_CFG_ESRC_DATA0_EXTINT_FLAGS_GNSSUTC_GET(val) \
    (U)(((val)&UBX_CFG_ESRC_DATA0_EXTINT_FLAGS_GNSSUTC_MASK) >> 1) //!< Get gnssUtc from bitmask flags
...
If the receiver is sent a valid new configuration, it will respond with a <rc href=\"UBX-ACK-ACK'>UBX-ACK-ACK</rc>
message and immediately change to the new configuration. Otherwise the receiver will reject the request, by issuing a
<rc href=\"UBX-ACK-NAK'>UBX-ACK-NAK</rc> and continuing operation with the previous configuration.
The number of tracking channels in use must not exceed the number of tracking channels available in hardware, and the
sum of all reserved tracking channels needs to be less than or equal to the number of tracking channels in use.
Additionally, the maximum number of tracking channels used for the specific GNSS system must be greater or equal to the
number of reserved tracking channels.
See section <r href=\"CFG-GNSS-DESC'>GNSS Configuration</r> for a discussion of the use of this message and section <r
href=\"DESC-SVNumbering-UBX'>Satellite Numbering</r> for a description of the GNSS IDs available.
Configuration specific to the GNSS system can be done via other messages (e.g. <rc
href=\"UBX-CFG-SBAS'>UBX-CFG-SBAS</rc>).
Note that GLONASS or BeiDou operation cannot be selected when the receiver is configured to operate in Power Save Mode
(using <rc href=\"UBX-CFG-RXM'>UBX-CFG-RXM</rc>).
...
#define UBX_CFG_GNSS_DATA0_GNSSID_FLAGS_ENABLE_GET(val) \
    (U)(((val)&UBX_CFG_GNSS_DATA0_GNSSID_FLAGS_ENABLE_MASK) >> 0)  //!< Get enable from bitmask flags
#define UBX_CFG_GNSS_DATA0_GNSSID_FLAGS_SIGCFGMASK_MASK 0x00FF0000 //!< Mask for field sigCfgMask in bitmask flags
#define UBX_CFG_GNSS_DATA0_GNSSID_FLAGS_SIGCFGMASK_GET(val) \
    (U)(((val)&UBX_CFG_GNSS_DATA0_GNSSID_FLAGS_SIGCFGMASK_MASK) >> 16) //!< Get sigCfgMask from bitmask flags
...
typedef struct UBX_CFG_INF_POLLID_s {
    U1 protocolID; //!< Protocol Identifier, identifying the output protocol for this Poll Request. The following are
...
The value of infMsgMask[x] below are that each bit represents one of the INF class messages (Bit 0 for ERROR, Bit 1 for
WARNING and so on.). For a complete list, see the <rc href=\"UBX-INF'>Message Class INF</rc>. Several configurations can
be concatenated to one input message. In this case the payload length can be a multiple of the normal length. Output
messages from the module contain only one configuration unit. Note that I/O Ports 1 and 2 correspond to serial ports 1
and 2. I/O port 0 is DDC. I/O port 3 is USB. I/O port 4 is SPI. I/O port 5 is reserved for future use.
...
typedef struct UBX_CFG_INF_DATA1_PROTOCOLID_s {
    U1 protocolID; //!< Protocol Identifier, identifying for which protocol the configuration is set/get. The following
                   //! are valid Protocol Identifiers:
...
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_ERROR_GET(val) \
    (U)(((val)&UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_ERROR_MASK) >> 0) //!< Get ERROR from bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_WARNING_MASK 0x02 //!< Mask for field WARNING in bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_WARNING_GET(val) \
    (U)(((val)&UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_WARNING_MASK) >> 1) //!< Get WARNING from bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_NOTICE_MASK 0x04 //!< Mask for field NOTICE in bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_NOTICE_GET(val) \
    (U)(((val)&UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_NOTICE_MASK) >> 2) //!< Get NOTICE from bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_TEST_MASK 0x08            //!< Mask for field TEST in bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_TEST_GET(val) \
    (U)(((val)&UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_TEST_MASK) >> 3) //!< Get TEST from bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_DEBUG_MASK 0x10         //!< Mask for field DEBUG in bitmask infMsgMask
#define UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_DEBUG_GET(val) \
    (U)(((val)&UBX_CFG_INF_DATA1_PROTOCOLID_INFMSGMASK_DEBUG_MASK) >> 4) //!< Get DEBUG from bitmask infMsgMask
...
#define UBX_CFG_ITFM_DATA0_CONFIG_BBTHRESHOLD_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG_BBTHRESHOLD_MASK) >> 0) //!< Get bbThreshold from bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_CWTHRESHOLD_MASK 0x000001F0    //!< Mask for field cwThreshold in bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_CWTHRESHOLD_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG_CWTHRESHOLD_MASK) >> 4) //!< Get cwThreshold from bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_ALGORITHMBITS_MASK 0x7FFFFE00  //!< Mask for field algorithmBits in bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_ALGORITHMBITS_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG_ALGORITHMBITS_MASK) >> 9) //!< Get algorithmBits from bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_ENABLE_MASK 0x80000000           //!< Mask for field enable in bitmask config
#define UBX_CFG_ITFM_DATA0_CONFIG_ENABLE_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG_ENABLE_MASK) >> 31) //!< Get enable from bitmask config
...
#define UBX_CFG_ITFM_DATA0_CONFIG2_GENERALBITS_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG2_GENERALBITS_MASK) >> 0) //!< Get generalBits from bitmask config2
#define UBX_CFG_ITFM_DATA0_CONFIG2_ANTSETTING_MASK 0x00003000     //!< Mask for field antSetting in bitmask config2
#define UBX_CFG_ITFM_DATA0_CONFIG2_ANTSETTING_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG2_ANTSETTING_MASK) >> 12) //!< Get antSetting from bitmask config2
#define UBX_CFG_ITFM_DATA0_CONFIG2_ENABLE2_MASK 0x00004000        //!< Mask for field enable2 in bitmask config2
#define UBX_CFG_ITFM_DATA0_CONFIG2_ENABLE2_GET(val) \
    (U)(((val)&UBX_CFG_ITFM_DATA0_CONFIG2_ENABLE2_MASK) >> 14) //!< Get enable2 from bitmask config2
...
Position entries can be filtered based on time difference, position difference or current speed thresholds. Position and
speed filtering also have a minimum time interval.
A position is logged if any of the thresholds are exceeded. If a threshold is set to zero it is ignored. The maximum
rate of position logging is 1Hz.
The filter settings will only be applied if the 'applyAllFilterSettings' flag is set. This enables recording to be
enabled/disabled without affecting the other settings.
...
typedef struct UBX_CFG_LOGFILTER_DATA0_s {
    ...
    X1 flags;             //!< Flags
    U2 minInterval;       //!< Minimum time interval between logged positions (0 = not set). <b>This is only applied in
                          //! combination with the speed and/or position thresholds</b>
    U2 timeThreshold;     //!< If the time difference is greater than the threshold then the position is logged (0 = not
                          //! set).
    U2 speedThreshold;    //!< If the current speed is greater than the threshold then the position is logged (0 = not
                          //! set). minInterval also applies
    U4 positionThreshold; //!< If the 3D position difference is greater than the threshold then the position is logged
                          //!(0 = not set). minInterval also applies
...
#define UBX_CFG_LOGFILTER_DATA0_FLAGS_RECORDENABLED_GET(val) \
    (U)(((val)&UBX_CFG_LOGFILTER_DATA0_FLAGS_RECORDENABLED_MASK) >> 0) //!< Get recordEnabled from bitmask flags
#define UBX_CFG_LOGFILTER_DATA0_FLAGS_PSMONCEPERWAKUPENABLED_MASK \
    0x02 //!< Mask for field psmOncePerWakupEnabled in bitmask flags
#define UBX_CFG_LOGFILTER_DATA0_FLAGS_PSMONCEPERWAKUPENABLED_GET(val)        \
    (U)(((val)&UBX_CFG_LOGFILTER_DATA0_FLAGS_PSMONCEPERWAKUPENABLED_MASK) >> \
        1) //!< Get psmOncePerWakupEnabled from bitmask flags
#define UBX_CFG_LOGFILTER_DATA0_FLAGS_APPLYALLFILTERSETTINGS_MASK \
    0x04 //!< Mask for field applyAllFilterSettings in bitmask flags
#define UBX_CFG_LOGFILTER_DATA0_FLAGS_APPLYALLFILTERSETTINGS_GET(val)        \
    (U)(((val)&UBX_CFG_LOGFILTER_DATA0_FLAGS_APPLYALLFILTERSETTINGS_MASK) >> \
        2) //!< Get applyAllFilterSettings from bitmask flags
...
/*!
Set Message Rate(s)
Set/Get message rate configuration (s) to/from the receiver. See also section <r href=\"CFG-MSG-DESC'>How to change
between protocols</r>.
* Send rate is relative to the event a message is registered on. For example, if the rate of a navigation message is set
to 2, the message is sent every second navigation solution. For configuring NMEA messages, the section <r
href=\"NMEA_ids'>NMEA Messages Overview</r> describes Class and Identifier numbers used.
...
/*!
Set Message Rate
Set message rate configuration for the current port. See also section <r href=\"CFG-MSG-DESC'>How to change between
protocols</r>.
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-NAV5
with a payload as defined below.
...
See the <r href=\"CFG-NAV5-DESC'>Navigation Configuration Settings Description</r> for a detailed description of how
...
#define UBX_CFG_NAV5_DATA0_MASK_DYN_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_DYN_MASK) >> 0) //!< Get dyn from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_MINEL_MASK 0x0002      //!< Mask for field minEl in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_MINEL_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_MINEL_MASK) >> 1) //!< Get minEl from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_POSFIXMODE_MASK 0x0004   //!< Mask for field posFixMode in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_POSFIXMODE_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_POSFIXMODE_MASK) >> 2) //!< Get posFixMode from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_DRLIM_MASK 0x0008             //!< Mask for field drLim in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_DRLIM_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_DRLIM_MASK) >> 3) //!< Get drLim from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_POSMASK_MASK 0x0010      //!< Mask for field posMask in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_POSMASK_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_POSMASK_MASK) >> 4) //!< Get posMask from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_TIMEMASK_MASK 0x0020       //!< Mask for field timeMask in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_TIMEMASK_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_TIMEMASK_MASK) >> 5) //!< Get timeMask from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_STATICHOLDMASK_MASK 0x0040  //!< Mask for field staticHoldMask in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_STATICHOLDMASK_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_STATICHOLDMASK_MASK) >> 6) //!< Get staticHoldMask from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_DGPSMASK_MASK 0x0080              //!< Mask for field dgpsMask in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_DGPSMASK_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_DGPSMASK_MASK) >> 7) //!< Get dgpsMask from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_CNOTHRESHOLD_MASK 0x0100    //!< Mask for field cnoThreshold in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_CNOTHRESHOLD_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_CNOTHRESHOLD_MASK) >> 8) //!< Get cnoThreshold from bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_UTC_MASK 0x0400                 //!< Mask for field utc in bitmask mask
#define UBX_CFG_NAV5_DATA0_MASK_UTC_GET(val) \
    (U)(((val)&UBX_CFG_NAV5_DATA0_MASK_UTC_MASK) >> 10) //!< Get utc from bitmask mask
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-NAVX5
with a payload as defined below.
...
typedef struct UBX_CFG_NAVX5_DATA0_s {
    ...
    U2 wknRollover;  //!< GPS week rollover number; GPS week numbers will be set correctly from this week up to 1024
                     //! weeks after this week. Setting this to 0 reverts to firmware default.
    ...
    U2 aopOrbMaxErr; //!< Maximum acceptable (modeled) <i>AssistNow Autonomous</i> orbit error (valid range = 5..1000,
...
#define UBX_CFG_NAVX5_DATA0_MASK1_MINMAX_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_MINMAX_MASK) >> 2) //!< Get minMax from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_MINCNO_MASK 0x0008        //!< Mask for field minCno in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_MINCNO_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_MINCNO_MASK) >> 3) //!< Get minCno from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_INITIAL3DFIX_MASK 0x0040  //!< Mask for field initial3dfix in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_INITIAL3DFIX_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_INITIAL3DFIX_MASK) >> 6) //!< Get initial3dfix from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_WKNROLL_MASK 0x0200             //!< Mask for field wknRoll in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_WKNROLL_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_WKNROLL_MASK) >> 9) //!< Get wknRoll from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_ACKAID_MASK 0x0400         //!< Mask for field ackAid in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_ACKAID_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_ACKAID_MASK) >> 10) //!< Get ackAid from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_PPP_MASK 0x2000            //!< Mask for field ppp in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_PPP_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_PPP_MASK) >> 13) //!< Get ppp from bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_AOP_MASK 0x4000         //!< Mask for field aop in bitmask mask1
#define UBX_CFG_NAVX5_DATA0_MASK1_AOP_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK1_AOP_MASK) >> 14) //!< Get aop from bitmask mask1
...
#define UBX_CFG_NAVX5_DATA0_MASK2_ADR_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_MASK2_ADR_MASK) >> 6) //!< Get adr from bitmask mask2
...
#define UBX_CFG_NAVX5_DATA0_AOPCFG_USEAOP_GET(val) \
    (U)(((val)&UBX_CFG_NAVX5_DATA0_AOPCFG_USEAOP_MASK) >> 0) //!< Get useAOP from bitmask aopCfg
...
Set/Get the <r href=\"NMEA_overview'>NMEA protocol</r> configuration. See section <r href=\"CFG-NMEA-DESC'>NMEA Protocol
Configuration</r> for a detailed description of the configuration effects on NMEA output.

   \note: This message version is provided for backwards compatibility only. Use the last version listed below instead
(its fields are backwards compatible with this version, it just has extra fields defined).
...
#define UBX_CFG_NMEA_DATA0_FILTER_POSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_POSFILT_MASK) >> 0) //!< Get posFilt from bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_MSKPOSFILT_MASK 0x02       //!< Mask for field mskPosFilt in bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_MSKPOSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_MSKPOSFILT_MASK) >> 1) //!< Get mskPosFilt from bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_TIMEFILT_MASK 0x04            //!< Mask for field timeFilt in bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_TIMEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_TIMEFILT_MASK) >> 2) //!< Get timeFilt from bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_DATEFILT_MASK 0x08          //!< Mask for field dateFilt in bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_DATEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_DATEFILT_MASK) >> 3) //!< Get dateFilt from bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_GPSONLYFILTER_MASK 0x10     //!< Mask for field gpsOnlyFilter in bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_GPSONLYFILTER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_GPSONLYFILTER_MASK) >> 4) //!< Get gpsOnlyFilter from bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_TRACKFILT_MASK 0x20              //!< Mask for field trackFilt in bitmask filter
#define UBX_CFG_NMEA_DATA0_FILTER_TRACKFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FILTER_TRACKFILT_MASK) >> 5) //!< Get trackFilt from bitmask filter
...
#define UBX_CFG_NMEA_DATA0_FLAGS_COMPAT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FLAGS_COMPAT_MASK) >> 0) //!< Get compat from bitmask flags
#define UBX_CFG_NMEA_DATA0_FLAGS_CONSIDER_MASK 0x02        //!< Mask for field consider in bitmask flags
#define UBX_CFG_NMEA_DATA0_FLAGS_CONSIDER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA0_FLAGS_CONSIDER_MASK) >> 1) //!< Get consider from bitmask flags
...
typedef struct UBX_CFG_NMEA_DATA1_s {
    ...
    U1 numSV;        //!< Maximum Number of SVs to report per TalkerId.
    X1 flags;        //!< flags
    X4 gnssToFilter; //!< Filters out satellites based on their GNSS. If a bitfield is enabled, the corresponding
                     //! satellites will be not output.
    U1 svNumbering;  //!< Configures the display of satellites that do not have an NMEA-defined value.
    U1 mainTalkerId; //!< By default the main Talker ID (i.e. the Talker ID used for all messages other than GSV) is
                     //! determined by the GNSS assignment of the receiver's channels (see <rc
    //! href=\"UBX-CFG-GNSS'>UBX-CFG-GNSS</rc>).
...
#define UBX_CFG_NMEA_DATA1_FILTER_POSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_POSFILT_MASK) >> 0) //!< Get posFilt from bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_MSKPOSFILT_MASK 0x02       //!< Mask for field mskPosFilt in bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_MSKPOSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_MSKPOSFILT_MASK) >> 1) //!< Get mskPosFilt from bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_TIMEFILT_MASK 0x04            //!< Mask for field timeFilt in bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_TIMEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_TIMEFILT_MASK) >> 2) //!< Get timeFilt from bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_DATEFILT_MASK 0x08          //!< Mask for field dateFilt in bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_DATEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_DATEFILT_MASK) >> 3) //!< Get dateFilt from bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_GPSONLYFILTER_MASK 0x10     //!< Mask for field gpsOnlyFilter in bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_GPSONLYFILTER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_GPSONLYFILTER_MASK) >> 4) //!< Get gpsOnlyFilter from bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_TRACKFILT_MASK 0x20              //!< Mask for field trackFilt in bitmask filter
#define UBX_CFG_NMEA_DATA1_FILTER_TRACKFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FILTER_TRACKFILT_MASK) >> 5) //!< Get trackFilt from bitmask filter
...
#define UBX_CFG_NMEA_DATA1_FLAGS_COMPAT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FLAGS_COMPAT_MASK) >> 0) //!< Get compat from bitmask flags
#define UBX_CFG_NMEA_DATA1_FLAGS_CONSIDER_MASK 0x02        //!< Mask for field consider in bitmask flags
#define UBX_CFG_NMEA_DATA1_FLAGS_CONSIDER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_FLAGS_CONSIDER_MASK) >> 1) //!< Get consider from bitmask flags
...
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_GPS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_GNSSTOFILTER_GPS_MASK) >> 0) //!< Get gps from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_SBAS_MASK 0x02         //!< Mask for field sbas in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_SBAS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_GNSSTOFILTER_SBAS_MASK) >> 1) //!< Get sbas from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_QZSS_MASK 0x10          //!< Mask for field qzss in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_QZSS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_GNSSTOFILTER_QZSS_MASK) >> 4) //!< Get qzss from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_GLONASS_MASK 0x20       //!< Mask for field glonass in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_GLONASS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_GNSSTOFILTER_GLONASS_MASK) >> 5) //!< Get glonass from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_BEIDOU_MASK 0x40           //!< Mask for field beidou in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA1_GNSSTOFILTER_BEIDOU_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA1_GNSSTOFILTER_BEIDOU_MASK) >> 6) //!< Get beidou from bitmask gnssToFilter
...
#define UBX_CFG_NMEA_DATA2_FILTER_POSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_POSFILT_MASK) >> 0) //!< Get posFilt from bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_MSKPOSFILT_MASK 0x02       //!< Mask for field mskPosFilt in bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_MSKPOSFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_MSKPOSFILT_MASK) >> 1) //!< Get mskPosFilt from bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_TIMEFILT_MASK 0x04            //!< Mask for field timeFilt in bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_TIMEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_TIMEFILT_MASK) >> 2) //!< Get timeFilt from bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_DATEFILT_MASK 0x08          //!< Mask for field dateFilt in bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_DATEFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_DATEFILT_MASK) >> 3) //!< Get dateFilt from bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_GPSONLYFILTER_MASK 0x10     //!< Mask for field gpsOnlyFilter in bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_GPSONLYFILTER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_GPSONLYFILTER_MASK) >> 4) //!< Get gpsOnlyFilter from bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_TRACKFILT_MASK 0x20              //!< Mask for field trackFilt in bitmask filter
#define UBX_CFG_NMEA_DATA2_FILTER_TRACKFILT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FILTER_TRACKFILT_MASK) >> 5) //!< Get trackFilt from bitmask filter
...
#define UBX_CFG_NMEA_DATA2_FLAGS_COMPAT_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FLAGS_COMPAT_MASK) >> 0) //!< Get compat from bitmask flags
#define UBX_CFG_NMEA_DATA2_FLAGS_CONSIDER_MASK 0x02        //!< Mask for field consider in bitmask flags
#define UBX_CFG_NMEA_DATA2_FLAGS_CONSIDER_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FLAGS_CONSIDER_MASK) >> 1) //!< Get consider from bitmask flags
#define UBX_CFG_NMEA_DATA2_FLAGS_LIMIT82_MASK 0x04           //!< Mask for field limit82 in bitmask flags
#define UBX_CFG_NMEA_DATA2_FLAGS_LIMIT82_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_FLAGS_LIMIT82_MASK) >> 2) //!< Get limit82 from bitmask flags
...
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_GPS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_GNSSTOFILTER_GPS_MASK) >> 0) //!< Get gps from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_SBAS_MASK 0x02         //!< Mask for field sbas in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_SBAS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_GNSSTOFILTER_SBAS_MASK) >> 1) //!< Get sbas from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_QZSS_MASK 0x10          //!< Mask for field qzss in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_QZSS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_GNSSTOFILTER_QZSS_MASK) >> 4) //!< Get qzss from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_GLONASS_MASK 0x20       //!< Mask for field glonass in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_GLONASS_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_GNSSTOFILTER_GLONASS_MASK) >> 5) //!< Get glonass from bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_BEIDOU_MASK 0x40           //!< Mask for field beidou in bitmask gnssToFilter
#define UBX_CFG_NMEA_DATA2_GNSSTOFILTER_BEIDOU_GET(val) \
    (U)(((val)&UBX_CFG_NMEA_DATA2_GNSSTOFILTER_BEIDOU_MASK) >> 6) //!< Get beidou from bitmask gnssToFilter
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-ODO
with a payload as defined below.
...
#define UBX_CFG_ODO_DATA1_FLAGS_USEODO_GET(val) \
    (U)(((val)&UBX_CFG_ODO_DATA1_FLAGS_USEODO_MASK) >> 0) //!< Get useODO from bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_USECOG_MASK 0x02          //!< Mask for field useCOG in bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_USECOG_GET(val) \
    (U)(((val)&UBX_CFG_ODO_DATA1_FLAGS_USECOG_MASK) >> 1) //!< Get useCOG from bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_OUTLPVEL_MASK 0x04        //!< Mask for field outLPVel in bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_OUTLPVEL_GET(val) \
    (U)(((val)&UBX_CFG_ODO_DATA1_FLAGS_OUTLPVEL_MASK) >> 2) //!< Get outLPVel from bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_OUTLPCOG_MASK 0x08          //!< Mask for field outLPCog in bitmask flags
#define UBX_CFG_ODO_DATA1_FLAGS_OUTLPCOG_GET(val) \
    (U)(((val)&UBX_CFG_ODO_DATA1_FLAGS_OUTLPCOG_MASK) >> 3) //!< Get outLPCog from bitmask flags
...
#define UBX_CFG_ODO_DATA1_ODOCFG_PROFILE_GET(val) \
    (U)(((val)&UBX_CFG_ODO_DATA1_ODOCFG_PROFILE_MASK) >> 0) //!< Get profile from bitmask odoCfg
...
typedef struct UBX_CFG_PM2_DATA0_s {
    ...
    U1 reserved1;          //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U1 maxStartupStateDur; //!< Maximum time to spend in <i>Acquisition</i> state. If 0: bound disabled (see <r
                           //! href=\"PM-DESC-MAXSTARTUPSTATEDUR'>maxStartupStateDur</r>). (Only supported in <r
    //! href='ProtocolVersions'>protocol versions 17+</r>)
    U1 reserved2;     //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    X4 flags;         //!< PSM configuration flags
    U4 updatePeriod;  //!< Position update period. If set to 0, the receiver will never retry a fix and it will wait for
                      //! external events
    U4 searchPeriod;  //!< Acquisition retry period if previously failed. If set to 0, the receiver will never retry a
                      //! startup
...
#define UBX_CFG_PM2_DATA0_FLAGS_EXTINTSEL_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_EXTINTSEL_MASK) >> 4) //!< Get extintSel from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_EXTINTWAKE_MASK 0x00000020   //!< Mask for field extintWake in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_EXTINTWAKE_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_EXTINTWAKE_MASK) >> 5) //!< Get extintWake from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_EXTINTBACKUP_MASK 0x00000040  //!< Mask for field extintBackup in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_EXTINTBACKUP_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_EXTINTBACKUP_MASK) >> 6) //!< Get extintBackup from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_LIMITPEAKCURR_MASK 0x00000300   //!< Mask for field limitPeakCurr in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_LIMITPEAKCURR_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_LIMITPEAKCURR_MASK) >> 8) //!< Get limitPeakCurr from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_WAITTIMEFIX_MASK 0x00000400      //!< Mask for field waitTimeFix in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_WAITTIMEFIX_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_WAITTIMEFIX_MASK) >> 10) //!< Get waitTimeFix from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_UPDATERTC_MASK 0x00000800       //!< Mask for field updateRTC in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_UPDATERTC_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_UPDATERTC_MASK) >> 11) //!< Get updateRTC from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_UPDATEEPH_MASK 0x00001000     //!< Mask for field updateEPH in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_UPDATEEPH_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_UPDATEEPH_MASK) >> 12) //!< Get updateEPH from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_DONOTENTEROFF_MASK 0x00010000 //!< Mask for field doNotEnterOff in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_DONOTENTEROFF_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_DONOTENTEROFF_MASK) >> 16) //!< Get doNotEnterOff from bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_MODE_MASK 0x00060000              //!< Mask for field mode in bitmask flags
#define UBX_CFG_PM2_DATA0_FLAGS_MODE_GET(val) \
    (U)(((val)&UBX_CFG_PM2_DATA0_FLAGS_MODE_MASK) >> 17) //!< Get mode from bitmask flags
...
Sending this message with a port ID as payload results in having the receiver return the configuration for the specified
port.
...
the normal length (see the other versions of CFG-PRT). Output messages from the module contain only one configuration
unit.
Note that this message can affect baud rate and other transmission parameters. Because there may be messages queued for
transmission there may be uncertainty about which protocol applies to such messages. In addition a message currently in
transmission may be corrupted by a protocol change. Host data reception paramaters may have to be changed to be able to
receive future messages, including the acknowledge message resulting from the CFG-PRT message.
...
typedef struct UBX_CFG_PRT_UART_s {
    U1 portID; //!< Port Identifier Number (see <r href=\"CFG-PRT-DESC'>Serial Communication Ports Description</r> for
               //! valid UART port IDs)
    U1 reserved1;    //!< <r href='DESC-UBX-Reserved'>Reserved</r>
...
#define UBX_CFG_PRT_UART_TXREADY_EN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_TXREADY_EN_MASK) >> 0) //!< Get en from bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_POL_MASK 0x00000002   //!< Mask for field pol in bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_POL_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_TXREADY_POL_MASK) >> 1) //!< Get pol from bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_PIN_MASK 0x0000007C    //!< Mask for field pin in bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_PIN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_TXREADY_PIN_MASK) >> 2) //!< Get pin from bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_THRES_MASK 0x0000FF80  //!< Mask for field thres in bitmask txReady
#define UBX_CFG_PRT_UART_TXREADY_THRES_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_TXREADY_THRES_MASK) >> 7) //!< Get thres from bitmask txReady
...
#define UBX_CFG_PRT_UART_MODE_CHARLEN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_MODE_CHARLEN_MASK) >> 6) //!< Get charLen from bitmask mode
#define UBX_CFG_PRT_UART_MODE_PARITY_MASK 0x00000E00     //!< Mask for field parity in bitmask mode
#define UBX_CFG_PRT_UART_MODE_PARITY_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_MODE_PARITY_MASK) >> 9) //!< Get parity from bitmask mode
#define UBX_CFG_PRT_UART_MODE_NSTOPBITS_MASK 0x00003000 //!< Mask for field nStopBits in bitmask mode
#define UBX_CFG_PRT_UART_MODE_NSTOPBITS_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_MODE_NSTOPBITS_MASK) >> 12) //!< Get nStopBits from bitmask mode
...
#define UBX_CFG_PRT_UART_INPROTOMASK_INUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_INPROTOMASK_INUBX_MASK) >> 0) //!< Get inUbx from bitmask inProtoMask
#define UBX_CFG_PRT_UART_INPROTOMASK_INNMEA_MASK 0x0002       //!< Mask for field inNmea in bitmask inProtoMask
#define UBX_CFG_PRT_UART_INPROTOMASK_INNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_INPROTOMASK_INNMEA_MASK) >> 1) //!< Get inNmea from bitmask inProtoMask
#define UBX_CFG_PRT_UART_INPROTOMASK_INRTCM_MASK 0x0004        //!< Mask for field inRtcm in bitmask inProtoMask
#define UBX_CFG_PRT_UART_INPROTOMASK_INRTCM_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_INPROTOMASK_INRTCM_MASK) >> 2) //!< Get inRtcm from bitmask inProtoMask
...
#define UBX_CFG_PRT_UART_OUTPROTOMASK_OUTUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_OUTPROTOMASK_OUTUBX_MASK) >> 0) //!< Get outUbx from bitmask outProtoMask
#define UBX_CFG_PRT_UART_OUTPROTOMASK_OUTNMEA_MASK 0x0002       //!< Mask for field outNmea in bitmask outProtoMask
#define UBX_CFG_PRT_UART_OUTPROTOMASK_OUTNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_OUTPROTOMASK_OUTNMEA_MASK) >> 1) //!< Get outNmea from bitmask outProtoMask
...
#define UBX_CFG_PRT_UART_FLAGS_EXTENDEDTXTIMEOUT_GET(val) \
    (U)(((val)&UBX_CFG_PRT_UART_FLAGS_EXTENDEDTXTIMEOUT_MASK) >> 1) //!< Get extendedTxTimeout from bitmask flags
...
#define UBX_CFG_PRT_USB_TXREADY_EN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_TXREADY_EN_MASK) >> 0) //!< Get en from bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_POL_MASK 0x00000002   //!< Mask for field pol in bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_POL_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_TXREADY_POL_MASK) >> 1) //!< Get pol from bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_PIN_MASK 0x0000007C    //!< Mask for field pin in bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_PIN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_TXREADY_PIN_MASK) >> 2) //!< Get pin from bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_THRES_MASK 0x0000FF80  //!< Mask for field thres in bitmask txReady
#define UBX_CFG_PRT_USB_TXREADY_THRES_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_TXREADY_THRES_MASK) >> 7) //!< Get thres from bitmask txReady
...
#define UBX_CFG_PRT_USB_INPROTOMASK_INUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_INPROTOMASK_INUBX_MASK) >> 0) //!< Get inUbx from bitmask inProtoMask
#define UBX_CFG_PRT_USB_INPROTOMASK_INNMEA_MASK 0x0002       //!< Mask for field inNmea in bitmask inProtoMask
#define UBX_CFG_PRT_USB_INPROTOMASK_INNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_INPROTOMASK_INNMEA_MASK) >> 1) //!< Get inNmea from bitmask inProtoMask
#define UBX_CFG_PRT_USB_INPROTOMASK_INRTCM_MASK 0x0004        //!< Mask for field inRtcm in bitmask inProtoMask
#define UBX_CFG_PRT_USB_INPROTOMASK_INRTCM_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_INPROTOMASK_INRTCM_MASK) >> 2) //!< Get inRtcm from bitmask inProtoMask
...
#define UBX_CFG_PRT_USB_OUTPROTOMASK_OUTUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_OUTPROTOMASK_OUTUBX_MASK) >> 0) //!< Get outUbx from bitmask outProtoMask
#define UBX_CFG_PRT_USB_OUTPROTOMASK_OUTNMEA_MASK 0x0002       //!< Mask for field outNmea in bitmask outProtoMask
#define UBX_CFG_PRT_USB_OUTPROTOMASK_OUTNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_USB_OUTPROTOMASK_OUTNMEA_MASK) >> 1) //!< Get outNmea from bitmask outProtoMask
...
#define UBX_CFG_PRT_SPI_TXREADY_EN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_TXREADY_EN_MASK) >> 0) //!< Get en from bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_POL_MASK 0x00000002   //!< Mask for field pol in bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_POL_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_TXREADY_POL_MASK) >> 1) //!< Get pol from bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_PIN_MASK 0x0000007C    //!< Mask for field pin in bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_PIN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_TXREADY_PIN_MASK) >> 2) //!< Get pin from bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_THRES_MASK 0x0000FF80  //!< Mask for field thres in bitmask txReady
#define UBX_CFG_PRT_SPI_TXREADY_THRES_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_TXREADY_THRES_MASK) >> 7) //!< Get thres from bitmask txReady
...
#define UBX_CFG_PRT_SPI_MODE_SPIMODE_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_MODE_SPIMODE_MASK) >> 1)  //!< Get spiMode from bitmask mode
#define UBX_CFG_PRT_SPI_MODE_FLOWCONTROL_MASK 0x00000040 //!< Mask for field flowControl in bitmask mode
#define UBX_CFG_PRT_SPI_MODE_FLOWCONTROL_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_MODE_FLOWCONTROL_MASK) >> 6) //!< Get flowControl from bitmask mode
#define UBX_CFG_PRT_SPI_MODE_FFCNT_MASK 0x00003F00          //!< Mask for field ffCnt in bitmask mode
#define UBX_CFG_PRT_SPI_MODE_FFCNT_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_MODE_FFCNT_MASK) >> 8) //!< Get ffCnt from bitmask mode
...
#define UBX_CFG_PRT_SPI_INPROTOMASK_INUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_INPROTOMASK_INUBX_MASK) >> 0) //!< Get inUbx from bitmask inProtoMask
#define UBX_CFG_PRT_SPI_INPROTOMASK_INNMEA_MASK 0x0002       //!< Mask for field inNmea in bitmask inProtoMask
#define UBX_CFG_PRT_SPI_INPROTOMASK_INNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_INPROTOMASK_INNMEA_MASK) >> 1) //!< Get inNmea from bitmask inProtoMask
#define UBX_CFG_PRT_SPI_INPROTOMASK_INRTCM_MASK 0x0004        //!< Mask for field inRtcm in bitmask inProtoMask
#define UBX_CFG_PRT_SPI_INPROTOMASK_INRTCM_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_INPROTOMASK_INRTCM_MASK) >> 2) //!< Get inRtcm from bitmask inProtoMask
...
#define UBX_CFG_PRT_SPI_OUTPROTOMASK_OUTUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_OUTPROTOMASK_OUTUBX_MASK) >> 0) //!< Get outUbx from bitmask outProtoMask
#define UBX_CFG_PRT_SPI_OUTPROTOMASK_OUTNMEA_MASK 0x0002       //!< Mask for field outNmea in bitmask outProtoMask
#define UBX_CFG_PRT_SPI_OUTPROTOMASK_OUTNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_OUTPROTOMASK_OUTNMEA_MASK) >> 1) //!< Get outNmea from bitmask outProtoMask
...
#define UBX_CFG_PRT_SPI_FLAGS_EXTENDEDTXTIMEOUT_GET(val) \
    (U)(((val)&UBX_CFG_PRT_SPI_FLAGS_EXTENDEDTXTIMEOUT_MASK) >> 1) //!< Get extendedTxTimeout from bitmask flags
...
#define UBX_CFG_PRT_DDC_TXREADY_EN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_TXREADY_EN_MASK) >> 0) //!< Get en from bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_POL_MASK 0x00000002   //!< Mask for field pol in bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_POL_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_TXREADY_POL_MASK) >> 1) //!< Get pol from bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_PIN_MASK 0x0000007C    //!< Mask for field pin in bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_PIN_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_TXREADY_PIN_MASK) >> 2) //!< Get pin from bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_THRES_MASK 0x0000FF80  //!< Mask for field thres in bitmask txReady
#define UBX_CFG_PRT_DDC_TXREADY_THRES_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_TXREADY_THRES_MASK) >> 7) //!< Get thres from bitmask txReady
...
#define UBX_CFG_PRT_DDC_MODE_SLAVEADDR_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_MODE_SLAVEADDR_MASK) >> 1) //!< Get slaveAddr from bitmask mode
...
#define UBX_CFG_PRT_DDC_INPROTOMASK_INUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_INPROTOMASK_INUBX_MASK) >> 0) //!< Get inUbx from bitmask inProtoMask
#define UBX_CFG_PRT_DDC_INPROTOMASK_INNMEA_MASK 0x0002       //!< Mask for field inNmea in bitmask inProtoMask
#define UBX_CFG_PRT_DDC_INPROTOMASK_INNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_INPROTOMASK_INNMEA_MASK) >> 1) //!< Get inNmea from bitmask inProtoMask
#define UBX_CFG_PRT_DDC_INPROTOMASK_INRTCM_MASK 0x0004        //!< Mask for field inRtcm in bitmask inProtoMask
#define UBX_CFG_PRT_DDC_INPROTOMASK_INRTCM_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_INPROTOMASK_INRTCM_MASK) >> 2) //!< Get inRtcm from bitmask inProtoMask
...
#define UBX_CFG_PRT_DDC_OUTPROTOMASK_OUTUBX_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_OUTPROTOMASK_OUTUBX_MASK) >> 0) //!< Get outUbx from bitmask outProtoMask
#define UBX_CFG_PRT_DDC_OUTPROTOMASK_OUTNMEA_MASK 0x0002       //!< Mask for field outNmea in bitmask outProtoMask
#define UBX_CFG_PRT_DDC_OUTPROTOMASK_OUTNMEA_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_OUTPROTOMASK_OUTNMEA_MASK) >> 1) //!< Get outNmea from bitmask outProtoMask
...
#define UBX_CFG_PRT_DDC_FLAGS_EXTENDEDTXTIMEOUT_GET(val) \
    (U)(((val)&UBX_CFG_PRT_DDC_FLAGS_EXTENDEDTXTIMEOUT_MASK) >> 1) //!< Get extendedTxTimeout from bitmask flags
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-RATE
with a payload as defined below
...
calculation of the navigation solution will always be aligned to the top of a second.
* The update rate has a direct influence on the power consumption. The more fixes that are required, the more CPU power
and communication resources are required.
* For most applications a 1 Hz update rate would be sufficient.
* When using Power Save Mode, measurement and navigation rate can differ from the values configured here. See <r
href=\"PM-DESC-MEASNAVRATE'>Measurement and navigation rate with Power Save Mode</r> for details.
...
typedef struct UBX_CFG_RATE_DATA0_s {
    ...
    U2 navRate; //!< Navigation Rate, in number of measurement cycles. This parameter cannot be changed, and must be set
                //! to 1.
...
#define UBX_CFG_RINV_DATA0_FLAGS_DUMP_GET(val) \
    (U)(((val)&UBX_CFG_RINV_DATA0_FLAGS_DUMP_MASK) >> 0) //!< Get dump from bitmask flags
#define UBX_CFG_RINV_DATA0_FLAGS_BINARY_MASK 0x2         //!< Mask for field binary in bitmask flags
#define UBX_CFG_RINV_DATA0_FLAGS_BINARY_GET(val) \
    (U)(((val)&UBX_CFG_RINV_DATA0_FLAGS_BINARY_MASK) >> 1) //!< Get binary from bitmask flags
...
* Older FW version will acknowledge this message but the acknowledge may not be sent completely before the receiver is
reset.
...
#define UBX_CFG_RST_DATA3_NAVBBRMASK_EPH_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_EPH_MASK) >> 0) //!< Get eph from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_ALM_MASK 0x0002        //!< Mask for field alm in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_ALM_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_ALM_MASK) >> 1) //!< Get alm from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_HEALTH_MASK 0x0004     //!< Mask for field health in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_HEALTH_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_HEALTH_MASK) >> 2) //!< Get health from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_KLOB_MASK 0x0008          //!< Mask for field klob in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_KLOB_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_KLOB_MASK) >> 3) //!< Get klob from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_POS_MASK 0x0010         //!< Mask for field pos in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_POS_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_POS_MASK) >> 4) //!< Get pos from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_CLKD_MASK 0x0020       //!< Mask for field clkd in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_CLKD_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_CLKD_MASK) >> 5) //!< Get clkd from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_OSC_MASK 0x0040         //!< Mask for field osc in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_OSC_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_OSC_MASK) >> 6) //!< Get osc from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_UTC_MASK 0x0080        //!< Mask for field utc in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_UTC_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_UTC_MASK) >> 7) //!< Get utc from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_RTC_MASK 0x0100        //!< Mask for field rtc in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_RTC_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_RTC_MASK) >> 8) //!< Get rtc from bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_AOP_MASK 0x8000        //!< Mask for field aop in bitmask navBbrMask
#define UBX_CFG_RST_DATA3_NAVBBRMASK_AOP_GET(val) \
    (U)(((val)&UBX_CFG_RST_DATA3_NAVBBRMASK_AOP_MASK) >> 15) //!< Get aop from bitmask navBbrMask
...
Note that Power Save Mode cannot be selected when the receiver is configured to process GLONASS signals (using <rc
href=\"UBX-CFG-GNSS'>CFG-GNSS</rc>).
...
/*!
SBAS Configuration
This message configures the SBAS receiver subsystem (i.e. WAAS, EGNOS, MSAS). See the <r href=\"CFG-SBAS-DESC'>SBAS
Configuration Settings Description</r> for a detailed description of how these settings affect receiver operation.
...
typedef struct UBX_CFG_SBAS_U5_s {
    X1 mode;      //!< SBAS Mode
    X1 usage;     //!< SBAS Usage
    U1 maxSBAS;   //!< Maximum Number of SBAS prioritized tracking channels (valid range: 0 - 3) to use (obsolete and
                  //! superseeded by UBX-CFG-GNSS in <r href='ProtocolVersions'>protocol versions 14+</r>).
...
#define UBX_CFG_SBAS_U5_MODE_ENABLED_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_MODE_ENABLED_MASK) >> 0) //!< Get enabled from bitmask mode
#define UBX_CFG_SBAS_U5_MODE_TEST_MASK 0x02             //!< Mask for field test in bitmask mode
#define UBX_CFG_SBAS_U5_MODE_TEST_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_MODE_TEST_MASK) >> 1) //!< Get test from bitmask mode
...
#define UBX_CFG_SBAS_U5_USAGE_RANGE_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_USAGE_RANGE_MASK) >> 0) //!< Get range from bitmask usage
#define UBX_CFG_SBAS_U5_USAGE_DIFFCORR_MASK 0x02       //!< Mask for field diffCorr in bitmask usage
#define UBX_CFG_SBAS_U5_USAGE_DIFFCORR_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_USAGE_DIFFCORR_MASK) >> 1) //!< Get diffCorr from bitmask usage
#define UBX_CFG_SBAS_U5_USAGE_INTEGRITY_MASK 0x04         //!< Mask for field integrity in bitmask usage
#define UBX_CFG_SBAS_U5_USAGE_INTEGRITY_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_USAGE_INTEGRITY_MASK) >> 2) //!< Get integrity from bitmask usage
...
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN152_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN152_MASK) >> 0) //!< Get PRN152 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN153_MASK 0x02          //!< Mask for field PRN153 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN153_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN153_MASK) >> 1) //!< Get PRN153 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN154_MASK 0x04          //!< Mask for field PRN154 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN154_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN154_MASK) >> 2) //!< Get PRN154 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN155_MASK 0x08          //!< Mask for field PRN155 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN155_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN155_MASK) >> 3) //!< Get PRN155 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN156_MASK 0x10          //!< Mask for field PRN156 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN156_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN156_MASK) >> 4) //!< Get PRN156 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN157_MASK 0x20          //!< Mask for field PRN157 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN157_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN157_MASK) >> 5) //!< Get PRN157 from bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN158_MASK 0x40          //!< Mask for field PRN158 in bitmask scanmode2
#define UBX_CFG_SBAS_U5_SCANMODE2_PRN158_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE2_PRN158_MASK) >> 6) //!< Get PRN158 from bitmask scanmode2
...
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN120_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN120_MASK) >> 0) //!< Get PRN120 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN121_MASK 0x00000002    //!< Mask for field PRN121 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN121_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN121_MASK) >> 1) //!< Get PRN121 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN122_MASK 0x00000004    //!< Mask for field PRN122 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN122_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN122_MASK) >> 2) //!< Get PRN122 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN123_MASK 0x00000008    //!< Mask for field PRN123 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN123_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN123_MASK) >> 3) //!< Get PRN123 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN124_MASK 0x00000010    //!< Mask for field PRN124 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN124_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN124_MASK) >> 4) //!< Get PRN124 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN125_MASK 0x00000020    //!< Mask for field PRN125 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN125_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN125_MASK) >> 5) //!< Get PRN125 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN126_MASK 0x00000040    //!< Mask for field PRN126 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN126_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN126_MASK) >> 6) //!< Get PRN126 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN127_MASK 0x00000080    //!< Mask for field PRN127 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN127_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN127_MASK) >> 7) //!< Get PRN127 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN128_MASK 0x00000100    //!< Mask for field PRN128 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN128_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN128_MASK) >> 8) //!< Get PRN128 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN129_MASK 0x00000200    //!< Mask for field PRN129 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN129_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN129_MASK) >> 9) //!< Get PRN129 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN130_MASK 0x00000400    //!< Mask for field PRN130 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN130_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN130_MASK) >> 10) //!< Get PRN130 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN131_MASK 0x00000800     //!< Mask for field PRN131 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN131_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN131_MASK) >> 11) //!< Get PRN131 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN132_MASK 0x00001000     //!< Mask for field PRN132 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN132_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN132_MASK) >> 12) //!< Get PRN132 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN133_MASK 0x00002000     //!< Mask for field PRN133 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN133_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN133_MASK) >> 13) //!< Get PRN133 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN134_MASK 0x00004000     //!< Mask for field PRN134 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN134_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN134_MASK) >> 14) //!< Get PRN134 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN135_MASK 0x00008000     //!< Mask for field PRN135 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN135_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN135_MASK) >> 15) //!< Get PRN135 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN136_MASK 0x00010000     //!< Mask for field PRN136 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN136_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN136_MASK) >> 16) //!< Get PRN136 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN137_MASK 0x00020000     //!< Mask for field PRN137 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN137_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN137_MASK) >> 17) //!< Get PRN137 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN138_MASK 0x00040000     //!< Mask for field PRN138 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN138_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN138_MASK) >> 18) //!< Get PRN138 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN139_MASK 0x00080000     //!< Mask for field PRN139 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN139_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN139_MASK) >> 19) //!< Get PRN139 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN140_MASK 0x00100000     //!< Mask for field PRN140 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN140_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN140_MASK) >> 20) //!< Get PRN140 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN141_MASK 0x00200000     //!< Mask for field PRN141 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN141_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN141_MASK) >> 21) //!< Get PRN141 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN142_MASK 0x00400000     //!< Mask for field PRN142 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN142_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN142_MASK) >> 22) //!< Get PRN142 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN143_MASK 0x00800000     //!< Mask for field PRN143 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN143_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN143_MASK) >> 23) //!< Get PRN143 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN144_MASK 0x01000000     //!< Mask for field PRN144 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN144_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN144_MASK) >> 24) //!< Get PRN144 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN145_MASK 0x02000000     //!< Mask for field PRN145 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN145_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN145_MASK) >> 25) //!< Get PRN145 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN146_MASK 0x04000000     //!< Mask for field PRN146 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN146_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN146_MASK) >> 26) //!< Get PRN146 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN147_MASK 0x08000000     //!< Mask for field PRN147 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN147_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN147_MASK) >> 27) //!< Get PRN147 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN148_MASK 0x10000000     //!< Mask for field PRN148 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN148_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN148_MASK) >> 28) //!< Get PRN148 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN149_MASK 0x20000000     //!< Mask for field PRN149 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN149_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN149_MASK) >> 29) //!< Get PRN149 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN150_MASK 0x40000000     //!< Mask for field PRN150 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN150_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN150_MASK) >> 30) //!< Get PRN150 from bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN151_MASK 0x80000000     //!< Mask for field PRN151 in bitmask scanmode1
#define UBX_CFG_SBAS_U5_SCANMODE1_PRN151_GET(val) \
    (U)(((val)&UBX_CFG_SBAS_U5_SCANMODE1_PRN151_MASK) >> 31) //!< Get PRN151 from bitmask scanmode1
...
typedef struct UBX_CFG_SMGR_DATA0_s {
    ...
    U1 reserved1[2];      //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U2 freqTolerance; //!< Limit of possible deviation from nominal before <rc href=\"UBX-TIM-TOS-DATA0'>TIM-TOS</rc>
                      //! indicates that frequency is out of tolerance
    U2 timeTolerance; //!< Limit of possible deviation from nominal before <rc href=\"UBX-TIM-TOS-DATA0'>TIM-TOS</rc>
                      //! indicates that time pulse is out of tolerance
    X2 messageCfg;    //!< Sync manager message configuration
    U2 maxSlewRate;   //!< Maximum slew rate, the maximum time correction that shall be applied between locked pulses in
                      //! corrective time pulse mode.
    X4 flags;         //!< Flags
...
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASINTERNAL_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASINTERNAL_MASK) >> 0) //!< Get measInternal from bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASGNSS_MASK 0x0002            //!< Mask for field measGNSS in bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASGNSS_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASGNSS_MASK) >> 1) //!< Get measGNSS from bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT0_MASK 0x0004     //!< Mask for field measEXTINT0 in bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT0_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT0_MASK) >> 2) //!< Get measEXTINT0 from bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT1_MASK 0x0008 //!< Mask for field measEXTINT1 in bitmask messageCfg
#define UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT1_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_MESSAGECFG_MEASEXTINT1_MASK) >> 3) //!< Get measEXTINT1 from bitmask messageCfg
...
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEINTERNAL_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_DISABLEINTERNAL_MASK) >> 0) //!< Get disableInternal from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEEXTERNAL_MASK 0x0002        //!< Mask for field disableExternal in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEEXTERNAL_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_DISABLEEXTERNAL_MASK) >> 1) //!< Get disableExternal from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_PREFERENCEMODE_MASK 0x0004         //!< Mask for field preferenceMode in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_PREFERENCEMODE_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_PREFERENCEMODE_MASK) >> 2) //!< Get preferenceMode from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEGNSS_MASK 0x0008            //!< Mask for field enableGNSS in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEGNSS_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ENABLEGNSS_MASK) >> 3) //!< Get enableGNSS from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT0_MASK 0x0010     //!< Mask for field enableEXTINT0 in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT0_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT0_MASK) >> 4) //!< Get enableEXTINT0 from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT1_MASK 0x0020        //!< Mask for field enableEXTINT1 in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT1_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ENABLEEXTINT1_MASK) >> 5) //!< Get enableEXTINT1 from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASINT_MASK 0x0040    //!< Mask for field enableHostMeasInt in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASINT_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASINT_MASK) >> 6) //!< Get enableHostMeasInt from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASEXT_MASK 0x0080 //!< Mask for field enableHostMeasExt in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASEXT_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ENABLEHOSTMEASEXT_MASK) >> 7) //!< Get enableHostMeasExt from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_USEANYFIX_MASK 0x0400                //!< Mask for field useAnyFix in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_USEANYFIX_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_USEANYFIX_MASK) >> 10)  //!< Get useAnyFix from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEMAXSLEWRATE_MASK 0x0800 //!< Mask for field disableMaxSlewRate in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEMAXSLEWRATE_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_DISABLEMAXSLEWRATE_MASK) >> 11) //!< Get disableMaxSlewRate from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ISSUEFREQWARNING_MASK 0x1000 //!< Mask for field issueFreqWarning in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ISSUEFREQWARNING_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ISSUEFREQWARNING_MASK) >> 12) //!< Get issueFreqWarning from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ISSUETIMEWARNING_MASK 0x2000 //!< Mask for field issueTimeWarning in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_ISSUETIMEWARNING_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_ISSUETIMEWARNING_MASK) >> 13) //!< Get issueTimeWarning from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_TPCOHERENT_MASK 0xc000               //!< Mask for field TPCoherent in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_TPCOHERENT_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_TPCOHERENT_MASK) >> 14) //!< Get TPCoherent from bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEOFFSET_MASK 0x10000     //!< Mask for field disableOffset in bitmask flags
#define UBX_CFG_SMGR_DATA0_FLAGS_DISABLEOFFSET_GET(val) \
    (U)(((val)&UBX_CFG_SMGR_DATA0_FLAGS_DISABLEOFFSET_MASK) >> 16) //!< Get disableOffset from bitmask flags
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type CFG-TMODE2
with a payload as defined below
...
See the <r href=\"CFG-TMODE-DESC'>Time Mode Description</r> for details. This message replaces the deprecated <rc
href=\"UBX-CFG-TMODE'>UBX-CFG-TMODE</rc> message.
...
#define UBX_CFG_TMODE2_DATA0_FLAGS_LLA_GET(val) \
    (U)(((val)&UBX_CFG_TMODE2_DATA0_FLAGS_LLA_MASK) >> 0) //!< Get lla from bitmask flags
#define UBX_CFG_TMODE2_DATA0_FLAGS_ALTINV_MASK 0x02       //!< Mask for field altInv in bitmask flags
#define UBX_CFG_TMODE2_DATA0_FLAGS_ALTINV_GET(val) \
    (U)(((val)&UBX_CFG_TMODE2_DATA0_FLAGS_ALTINV_MASK) >> 1) //!< Get altInv from bitmask flags
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a message of type <rc
href=\"UBX-CFG-TP5'>CFG-TP5</rc> with a payload as defined below for timepulse 0.
...
/*!
Poll Time Pulse Parameters
Sending this message to the receiver results in the receiver returning a message of type <rc
href=\"UBX-CFG-TP5'>CFG-TP5</rc> with a payload as defined below for the specified time pulse.
...
This message is used to get/set time pulse parameters. For more information see section <r href=\"CFG-TP5-DESC'>Time
pulse</r>.
...
#define UBX_CFG_TP5_DATA0_FLAGS_ACTIVE_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_ACTIVE_MASK) >> 0) //!< Get active from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_LOCKGPSFREQ_MASK 0x02     //!< Mask for field lockGpsFreq in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_LOCKGPSFREQ_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_LOCKGPSFREQ_MASK) >> 1) //!< Get lockGpsFreq from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_LOCKEDOTHERSET_MASK 0x04       //!< Mask for field lockedOtherSet in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_LOCKEDOTHERSET_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_LOCKEDOTHERSET_MASK) >> 2) //!< Get lockedOtherSet from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ISFREQ_MASK 0x08                  //!< Mask for field isFreq in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ISFREQ_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_ISFREQ_MASK) >> 3) //!< Get isFreq from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ISLENGTH_MASK 0x10        //!< Mask for field isLength in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ISLENGTH_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_ISLENGTH_MASK) >> 4) //!< Get isLength from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ALIGNTOTOW_MASK 0x20        //!< Mask for field alignToTow in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_ALIGNTOTOW_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_ALIGNTOTOW_MASK) >> 5) //!< Get alignToTow from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_POLARITY_MASK 0x40            //!< Mask for field polarity in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_POLARITY_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_POLARITY_MASK) >> 6) //!< Get polarity from bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_GRIDUTCGPS_MASK 0x80        //!< Mask for field gridUtcGps in bitmask flags
#define UBX_CFG_TP5_DATA0_FLAGS_GRIDUTCGPS_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA0_FLAGS_GRIDUTCGPS_MASK) >> 7) //!< Get gridUtcGps from bitmask flags
...
#define UBX_CFG_TP5_DATA1_FLAGS_ACTIVE_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_ACTIVE_MASK) >> 0) //!< Get active from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_LOCKGNSSFREQ_MASK 0x02    //!< Mask for field lockGnssFreq in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_LOCKGNSSFREQ_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_LOCKGNSSFREQ_MASK) >> 1) //!< Get lockGnssFreq from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_LOCKEDOTHERSET_MASK 0x04        //!< Mask for field lockedOtherSet in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_LOCKEDOTHERSET_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_LOCKEDOTHERSET_MASK) >> 2) //!< Get lockedOtherSet from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ISFREQ_MASK 0x08                  //!< Mask for field isFreq in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ISFREQ_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_ISFREQ_MASK) >> 3) //!< Get isFreq from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ISLENGTH_MASK 0x10        //!< Mask for field isLength in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ISLENGTH_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_ISLENGTH_MASK) >> 4) //!< Get isLength from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ALIGNTOTOW_MASK 0x20        //!< Mask for field alignToTow in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_ALIGNTOTOW_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_ALIGNTOTOW_MASK) >> 5) //!< Get alignToTow from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_POLARITY_MASK 0x40            //!< Mask for field polarity in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_POLARITY_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_POLARITY_MASK) >> 6) //!< Get polarity from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_GRIDUTCGNSS_MASK 0x780      //!< Mask for field gridUtcGnss in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_GRIDUTCGNSS_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_GRIDUTCGNSS_MASK) >> 7) //!< Get gridUtcGnss from bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_SYNCMODE_MASK 0x3800           //!< Mask for field syncMode in bitmask flags
#define UBX_CFG_TP5_DATA1_FLAGS_SYNCMODE_GET(val) \
    (U)(((val)&UBX_CFG_TP5_DATA1_FLAGS_SYNCMODE_MASK) >> 11) //!< Get syncMode from bitmask flags
...
This message configures how transmit time slots are defined for the receiver interfaces. These time slots are relative
to the chosen time pulse. A receiver that supports this message offers 3 time slots: nr. 0, 1 and 2. These time pulses
follow each other and their associated priorities decrease in this order. The end of each can be specified in this
message, the beginning is when the circularly previous slot ends (i.e. slot 0 starts when slot 2 finishes).
...
#define UBX_CFG_TXSLOT_DATA0_ENABLE_DDC_GET(val) \
    (U)(((val)&UBX_CFG_TXSLOT_DATA0_ENABLE_DDC_MASK) >> 0) //!< Get DDC from bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_UART1_MASK 0x00000002  //!< Mask for field UART1 in bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_UART1_GET(val) \
    (U)(((val)&UBX_CFG_TXSLOT_DATA0_ENABLE_UART1_MASK) >> 1) //!< Get UART1 from bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_UART2_MASK 0x00000004    //!< Mask for field UART2 in bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_UART2_GET(val) \
    (U)(((val)&UBX_CFG_TXSLOT_DATA0_ENABLE_UART2_MASK) >> 2) //!< Get UART2 from bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_USB_MASK 0x00000008      //!< Mask for field USB in bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_USB_GET(val) \
    (U)(((val)&UBX_CFG_TXSLOT_DATA0_ENABLE_USB_MASK) >> 3) //!< Get USB from bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_SPI_MASK 0x00000010    //!< Mask for field SPI in bitmask enable
#define UBX_CFG_TXSLOT_DATA0_ENABLE_SPI_GET(val) \
    (U)(((val)&UBX_CFG_TXSLOT_DATA0_ENABLE_SPI_MASK) >> 4) //!< Get SPI from bitmask enable
...
typedef struct UBX_CFG_USB_DATA0_s {
    U2 vendorID;     //!< Vendor ID. This field shall only be set to registered Vendor IDs. Changing this field requires
...
#define UBX_CFG_USB_DATA0_FLAGS_REENUM_GET(val) \
    (U)(((val)&UBX_CFG_USB_DATA0_FLAGS_REENUM_MASK) >> 0) //!< Get reEnum from bitmask flags
#define UBX_CFG_USB_DATA0_FLAGS_POWERMODE_MASK 0x0002     //!< Mask for field powerMode in bitmask flags
#define UBX_CFG_USB_DATA0_FLAGS_POWERMODE_GET(val) \
    (U)(((val)&UBX_CFG_USB_DATA0_FLAGS_POWERMODE_MASK) >> 1) //!< Get powerMode from bitmask flags
...
<rc href=\"UBX-ACK-ACK'>UBX-ACK-ACK</rc> or <rc href=\"UBX-ACK-NAK'>UBX-ACK-NAK</rc> are returned to indicate success or
failure.
This message does not handle activation of recording or filtering of log entries (see <rc
href=\"UBX-CFG-LOGFILTER'>UBX-CFG-LOGFILTER</rc>).
...
#define UBX_LOG_CREATE_CREATE0_LOGCFG_CIRCULAR_GET(val) \
    (U)(((val)&UBX_LOG_CREATE_CREATE0_LOGCFG_CIRCULAR_MASK) >> 0) //!< Get circular from bitmask logCfg
...
This message can be used to search a log for the index of the first entry less than or equal to the given time. This
index can then be used with the <rc href=\"UBX-LOG-RETRIEVE'>UBX-LOG-RETRIEVE</rc> message to provide time-based
...
Note:
* The reported maximum log size will be smaller than that originally specified in LOG-CREATE due to logging and
filestore implementation overheads.
* Log entries are compressed in a variable length fashion, so it may be difficult to predict log space usage with any
precision.
* There may be times when the receiver does not have an accurate time (e.g. if the week number is not yet known), in
which case some entries will not have a timestamp - this may result in the oldest/newest entry time values not taking
...
#define UBX_LOG_INFO_DATA0_STATUS_RECORDING_GET(val) \
    (U)(((val)&UBX_LOG_INFO_DATA0_STATUS_RECORDING_MASK) >> 3) //!< Get recording from bitmask status
#define UBX_LOG_INFO_DATA0_STATUS_INACTIVE_MASK 0x10           //!< Mask for field inactive in bitmask status
#define UBX_LOG_INFO_DATA0_STATUS_INACTIVE_GET(val) \
    (U)(((val)&UBX_LOG_INFO_DATA0_STATUS_INACTIVE_MASK) >> 4) //!< Get inactive from bitmask status
#define UBX_LOG_INFO_DATA0_STATUS_CIRCULAR_MASK 0x20          //!< Mask for field circular in bitmask status
#define UBX_LOG_INFO_DATA0_STATUS_CIRCULAR_GET(val) \
    (U)(((val)&UBX_LOG_INFO_DATA0_STATUS_CIRCULAR_MASK) >> 5) //!< Get circular from bitmask status
...
/*!
Request log data
This message is used to request logged data (log recording must first be disabled, see <rc
href=\"UBX-CFG-LOGFILTER'>UBX-CFG-LOGFILTER</rc>).
Log entries are returned in chronological order, using the messages <rc
href=\"UBX-LOG-RETRIEVEPOS'>UBX-LOG-RETRIEVEPOS</rc> and <rc href=\"UBX-LOG-RETRIEVESTRING'>UBX-LOG-RETRIEVESTRING</rc>.
The maximum number of entries that can be returned in response to a single UBX-LOG-RETRIEVE message is 256. If more
entries than this are required the mesage will need to be sent multiple times with different startNumbers. The retrieve
will be stopped if any UBX-LOG message is received. The speed of transfer can be maximized by using a high data rate and
...
This message can be used to store an arbitrary byte string in the on-board flash memory. The maximum length that can be
stored is 256 bytes.
...
This message is sent by a u-blox receiver to acknowledge the receipt of an assistance message. Acknowledgements are
enabled by setting the ackAiding parameter in the <rc href=\"UBX-CFG-NAVX5'>UBX-CFG-NAVX5</rc> message. See the
description of <r href=\"MGA-FlowControl'>flow control</r> for details.
...
This message is created by the AssistNow Offline service to deliver AssistNow Offline assistance to the receiver. See
the description of <r href=\"MGA-Offline'>AssistNow Offline</r> for details.
...
Poll the whole navigation data base. The receiver will send all available data from its internal database. The receiver
will indicate the finish of the transmission with a <rc href=\"UBX-MGA-ACK'>UBX-MGA-ACK</rc>. The msgPayloadStart field
of the UBX-MGA-ACK message will contain a U4 representing the number of UBX-MGA-DBD-DATA* messages sent.
...
Navigation database entry. The data fields are firmware specific. Transmission of this type of message will be
acknowledged by <rc href=\"UBX-MGA-ACK'>MGA-ACK</rc> messages, if acknowledgement has been enabled (see the description
of <r href=\"MGA-FlowControl'>flow control</r> for details).
...
This message is used to transfer a block of MGA-ANO data from host to the receiver. Upon reception of this message, the
receiver will write the payload data to its internal non-volatile memory (flash). Also, on reception of the first
MGA-FLASH-DATA message, the receiver will erase the flash allocated to storing any existing MGA-ANO data. The payload
can be up to 512 bytes. Payloads larger than this would exceed the receiver's internal buffering capabilities. The
receiver will ACK/NACK this message using the message alternatives given below. The host shall wait for an acknowledge
message before sending the next data block. See <r href=\"MGA-OfflineFlash'>Flash-based AssistNow Offline</r> for
details.
...
typedef struct UBX_MGA_FLASH_DATA_s {
    ...
    U2 sequence; //!< Message sequence number, starting at 0 and increamenting by 1 for each MGA-FLASH-DATA message
                 //! sent.
...
This message is used to tell the receiver that there are no more MGA-FLASH type 1 messages coming, and that it can do
any final internal operations needed to commit the data to flash as a background activity. A UBX-MGA-ACK message will be
sent at the end of this process. Note that there may be a delay of several seconds before the UBX-MGA-ACK for this
message is sent because of the time taken for this processing. See <r href=\"MGA-OfflineFlash'>Flash-based AssistNow
Offline</r> for details.
...
This message reports an ACK/NACK to the host for the last MGA-FLASH type 1 or type 2 message message received. See <r
href=\"MGA-OfflineFlash'>Flash-based AssistNow Offline</r> for details.
...
typedef struct UBX_MGA_FLASH_ACK_s {
    ...
    U1 ack;     //!< Acknowledgement type. 0 - ACK: Message received and written to flash. 1 - NACK: Problem with last
    //! message, re-transmission required (this only happens while acknowledging a UBX-MGA_FLASH_DATA message). 2
    //!- NACK: problem with last message, give up.
    U1 reserved1; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U2 sequence; //!< If acknowledging a UBX-MGA-FLASH-DATA message this is the Message sequence number being ack'ed. If
                 //! acknowledging a UBX-MGA-FLASH-STOP message it will be set to 0xffff.
...
This message allows the delivery of GLONASS ephemeris assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of GLONASS almanac assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
typedef struct UBX_MGA_GLO_ALM_s {
    ...
    I4 lambda;  //!< Longitude of the first (within the N-day) ascending node of satellite orbit in PC-90.02 coordinate
                //! system
...
This message allows the delivery of auxiliary GLONASS assistance (including the GLONASS time offsets to other GNSS
systems) to a receiver. See the description of <r href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of GPS ephemeris assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of GPS almanac assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of GPS health assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
typedef struct UBX_MGA_GPS_HEALTH_s {
    ...
    U1 reserved1[3];   //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U1 healthCode[32]; //!< Each byte represents a GPS SV (1-32). The 6 LSBs of each byte contains the 6 bit health code
...
/*!
GPS UTC Assistance
This message allows the delivery of GPS UTC assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of GPS ionospheric assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of initial position assistance to a receiver in cartesian ECEF coordinates. This
message is equivalent to the <rc href=\"UBX-MGA-INI-POS_LLH'>UBX-MGA-INI-POS_LLH</rc> message, except for the coordinate
system. See the description of <r href=\"MGA-Online'>AssistNow Online</r> for details.

   \note: Supplying position assistance that is inaccurate by more than the specified position accuracy, may lead to
substantially degraded receiver performance.
...
/*!
Initial Position Assistance
This message allows the delivery of initial position assistance to a receiver in WGS84 lat/long/alt coordinates. This
message is equivalent to the <rc href=\"UBX-MGA-INI-POS_XYZ'>UBX-MGA-INI-POS_XYZ</rc> message, except for the coordinate
system. See the description of <r href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of UTC time assistance to a receiver. This message is equivalent to the <rc
href=\"UBX-MGA-INI-TIME_GNSS'>UBX-MGA-INI-TIME_GNSS</rc> message, except for the time base. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.

   \note: Supplying time assistance that is inaccurate by more than the specified time accuracy, may lead to
substantially degraded receiver performance.
...
#define UBX_MGA_INI_TIME_UTC_REF_SOURCE_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_UTC_REF_SOURCE_MASK) >> 0) //!< Get source from bitmask ref
#define UBX_MGA_INI_TIME_UTC_REF_FALL_MASK 0x10            //!< Mask for field fall in bitmask ref
#define UBX_MGA_INI_TIME_UTC_REF_FALL_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_UTC_REF_FALL_MASK) >> 4) //!< Get fall from bitmask ref
#define UBX_MGA_INI_TIME_UTC_REF_LAST_MASK 0x20          //!< Mask for field last in bitmask ref
#define UBX_MGA_INI_TIME_UTC_REF_LAST_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_UTC_REF_LAST_MASK) >> 5) //!< Get last from bitmask ref
...
/*!
Initial Time Assistance
This message allows the delivery of time assistance to a receiver in a chosen GNSS timebase. This message is equivalent
to the <rc href=\"UBX-MGA-INI-TIME_UTC'>UBX-MGA-INI-TIME_UTC</rc> message, except for the time base. See the description
of <r href=\"MGA-Online'>AssistNow Online</r> for details.
...
#define UBX_MGA_INI_TIME_GNSS_REF_SOURCE_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_GNSS_REF_SOURCE_MASK) >> 0) //!< Get source from bitmask ref
#define UBX_MGA_INI_TIME_GNSS_REF_FALL_MASK 0x10            //!< Mask for field fall in bitmask ref
#define UBX_MGA_INI_TIME_GNSS_REF_FALL_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_GNSS_REF_FALL_MASK) >> 4) //!< Get fall from bitmask ref
#define UBX_MGA_INI_TIME_GNSS_REF_LAST_MASK 0x20          //!< Mask for field last in bitmask ref
#define UBX_MGA_INI_TIME_GNSS_REF_LAST_GET(val) \
    (U)(((val)&UBX_MGA_INI_TIME_GNSS_REF_LAST_MASK) >> 5) //!< Get last from bitmask ref
...
This message allows the delivery of clock drift assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.

   \note: Supplying clock drift assistance that is inaccurate by more than the specified accuracy, may lead to
substantially degraded receiver performance.
...
This message allows the delivery of external frequency assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.

   \note: Supplying external frequency assistance that is inaccurate by more than the specified accuracy, may lead to
substantially degraded receiver performance.
...
#define UBX_MGA_INI_FREQ_FLAGS_SOURCE_GET(val) \
    (U)(((val)&UBX_MGA_INI_FREQ_FLAGS_SOURCE_MASK) >> 0) //!< Get source from bitmask flags
#define UBX_MGA_INI_FREQ_FLAGS_FALL_MASK 0x10            //!< Mask for field fall in bitmask flags
#define UBX_MGA_INI_FREQ_FLAGS_FALL_GET(val) \
    (U)(((val)&UBX_MGA_INI_FREQ_FLAGS_FALL_MASK) >> 4) //!< Get fall from bitmask flags
...
This message allows the delivery of new Earth Orientation Parameters (EOP) to a receiver to improve AssistNow Autonomous
operation.
...
This message allows the delivery of QZSS ephemeris assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of QZSS almanac assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
This message allows the delivery of QZSS health assistance to a receiver. See the description of <r
href=\"MGA-Online'>AssistNow Online</r> for details.
...
typedef struct UBX_MGA_QZSS_HEALTH_s {
    U1 type;          //!< Message type. Set to 4 for this message (4 = health flags).
    U1 reserved1[3];  //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U1 healthCode[5]; //!< Each byte represents a QZSS SV (1-5). The 6 LSBs of each byte contains the 6 bit health code
...
This message reports GNSS system selection. It does this be means of bit masks in U1 fields. Each bit in a bit mask
corresponds to one GNSS system. Systems such as SBAS and QZSS are not reported. If systems such as SBAS/QZSS are related
...
typedef struct UBX_MON_GNSS_DATA0_s {
    ...
    X1 Default;   //!< A bit mask, saying which GNSS systems are enabled in the current efuse default configuration for
                  //! this receiver
...
#define UBX_MON_GNSS_DATA0_SUPPORTED_GPSSUP_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_SUPPORTED_GPSSUP_MASK) >> 0) //!< Get GPSSup from bitmask Supported
#define UBX_MON_GNSS_DATA0_SUPPORTED_GLONASSSUP_MASK 0x02      //!< Mask for field GlonassSup in bitmask Supported
#define UBX_MON_GNSS_DATA0_SUPPORTED_GLONASSSUP_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_SUPPORTED_GLONASSSUP_MASK) >> 1) //!< Get GlonassSup from bitmask Supported
#define UBX_MON_GNSS_DATA0_SUPPORTED_BEIDOUSUP_MASK 0x04           //!< Mask for field BeidouSup in bitmask Supported
#define UBX_MON_GNSS_DATA0_SUPPORTED_BEIDOUSUP_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_SUPPORTED_BEIDOUSUP_MASK) >> 2) //!< Get BeidouSup from bitmask Supported
...
#define UBX_MON_GNSS_DATA0_DEFAULT_GPSDEF_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_DEFAULT_GPSDEF_MASK) >> 0) //!< Get GPSDef from bitmask Default
#define UBX_MON_GNSS_DATA0_DEFAULT_GLONASSDEF_MASK 0x02      //!< Mask for field GlonassDef in bitmask Default
#define UBX_MON_GNSS_DATA0_DEFAULT_GLONASSDEF_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_DEFAULT_GLONASSDEF_MASK) >> 1) //!< Get GlonassDef from bitmask Default
#define UBX_MON_GNSS_DATA0_DEFAULT_BEIDOUDEF_MASK 0x04           //!< Mask for field BeidouDef in bitmask Default
#define UBX_MON_GNSS_DATA0_DEFAULT_BEIDOUDEF_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_DEFAULT_BEIDOUDEF_MASK) >> 2) //!< Get BeidouDef from bitmask Default
...
#define UBX_MON_GNSS_DATA0_ENABLED_GPSENA_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_ENABLED_GPSENA_MASK) >> 0) //!< Get GPSEna from bitmask Enabled
#define UBX_MON_GNSS_DATA0_ENABLED_GLONASSENA_MASK 0x02      //!< Mask for field GlonassEna in bitmask Enabled
#define UBX_MON_GNSS_DATA0_ENABLED_GLONASSENA_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_ENABLED_GLONASSENA_MASK) >> 1) //!< Get GlonassEna from bitmask Enabled
#define UBX_MON_GNSS_DATA0_ENABLED_BEIDOUENA_MASK 0x04           //!< Mask for field BeidouEna in bitmask Enabled
#define UBX_MON_GNSS_DATA0_ENABLED_BEIDOUENA_GET(val) \
    (U)(((val)&UBX_MON_GNSS_DATA0_ENABLED_BEIDOUENA_MASK) >> 2) //!< Get BeidouEna from bitmask Enabled
...
The first four parameters of this message represent the complex signal from the RF front end. The following rules of
thumb apply:
* The smaller the absolute value of the variable <c>ofsI</c> and <c>ofsQ</c>, the better.
* Ideally, the magnitude of the I-part (<c>magI</c>) and the Q-part (<c>magQ</c>) of the complex signal should be the
same.
...
typedef struct UBX_MON_HW2_DATA0_s {
    I1 ofsI; //!< Imbalance of I-part of complex signal, scaled (-128 = max. negative imbalance, 127 = max. positive
             //! imbalance)
    U1 magI; //!< Magnitude of I-part of complex signal, scaled (0 = no signal, 255 = max. magnitude)
    I1 ofsQ; //!< Imbalance of Q-part of complex signal, scaled (-128 = max. negative imbalance, 127 = max. positive
             //! imbalance)
    ...
    U1 reserved1[3]; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U4 lowLevCfg; //!< Low-level configuration (obsolete, only use this field if the message MON-LLC is not available in
                  //! your receiver)
    U1 reserved2[8]; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
...
/*!
Hardware Status
Status of different aspect of the hardware, such as Antenna, PIO/Peripheral Pins, Noise Level, Automatic Gain Control
(AGC)
...
#define UBX_MON_HW_TITLIS_FLAGS_RTCCALIB_GET(val) \
    (U)(((val)&UBX_MON_HW_TITLIS_FLAGS_RTCCALIB_MASK) >> 0) //!< Get rtcCalib from bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_SAFEBOOT_MASK 0x2           //!< Mask for field safeBoot in bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_SAFEBOOT_GET(val) \
    (U)(((val)&UBX_MON_HW_TITLIS_FLAGS_SAFEBOOT_MASK) >> 1) //!< Get safeBoot from bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_JAMMINGSTATE_MASK 0xC       //!< Mask for field jammingState in bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_JAMMINGSTATE_GET(val) \
    (U)(((val)&UBX_MON_HW_TITLIS_FLAGS_JAMMINGSTATE_MASK) >> 2) //!< Get jammingState from bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_XTALABSENT_MASK 0x10            //!< Mask for field xtalAbsent in bitmask flags
#define UBX_MON_HW_TITLIS_FLAGS_XTALABSENT_GET(val) \
    (U)(((val)&UBX_MON_HW_TITLIS_FLAGS_XTALABSENT_MASK) >> 4) //!< Get xtalAbsent from bitmask flags
...
The size of the message is determined by the number of ports 'N' the receiver supports, i.e. on u-blox 5 the number of
ports is 6.
...
//@{
#define UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_ACTIVATED_MASK \
    0x00000001 //!< Mask for field activated in bitmask patchInfo
#define UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_ACTIVATED_GET(val) \
    (U)(((val)&UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_ACTIVATED_MASK) >> 0) //!< Get activated from bitmask patchInfo
#define UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_LOCATION_MASK \
    0x00000006 //!< Mask for field location in bitmask patchInfo
#define UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_LOCATION_GET(val) \
    (U)(((val)&UBX_MON_PATCH_DATA0_PATCHINFO_PATCHINFO_LOCATION_MASK) >> 1) //!< Get location from bitmask patchInfo
...
#define UBX_MON_RXR_VER0_FLAGS_AWAKE_GET(val) \
    (U)(((val)&UBX_MON_RXR_VER0_FLAGS_AWAKE_MASK) >> 0) //!< Get awake from bitmask flags
...
This message reports the status of internal and external oscillators and sources as well as whether GNSS is used for
disciplining.
...
#define UBX_MON_SMGR_DATA0_INTOSC_INTOSCSTATE_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_INTOSC_INTOSCSTATE_MASK) >> 0) //!< Get intOscState from bitmask intOsc
#define UBX_MON_SMGR_DATA0_INTOSC_INTOSCCALIB_MASK 0x10          //!< Mask for field intOscCalib in bitmask intOsc
#define UBX_MON_SMGR_DATA0_INTOSC_INTOSCCALIB_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_INTOSC_INTOSCCALIB_MASK) >> 4) //!< Get intOscCalib from bitmask intOsc
#define UBX_MON_SMGR_DATA0_INTOSC_INTOSCDISC_MASK 0x20           //!< Mask for field intOscDisc in bitmask intOsc
#define UBX_MON_SMGR_DATA0_INTOSC_INTOSCDISC_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_INTOSC_INTOSCDISC_MASK) >> 5) //!< Get intOscDisc from bitmask intOsc
...
#define UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCSTATE_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCSTATE_MASK) >> 0) //!< Get extOscState from bitmask extOsc
#define UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCCALIB_MASK 0x10          //!< Mask for field extOscCalib in bitmask extOsc
#define UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCCALIB_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCCALIB_MASK) >> 4) //!< Get extOscCalib from bitmask extOsc
#define UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCDISC_MASK 0x20           //!< Mask for field extOscDisc in bitmask extOsc
#define UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCDISC_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTOSC_EXTOSCDISC_MASK) >> 5) //!< Get extOscDisc from bitmask extOsc
...
#define UBX_MON_SMGR_DATA0_GNSS_GNSSAVAIL_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_GNSS_GNSSAVAIL_MASK) >> 0) //!< Get gnssAvail from bitmask gnss
...
#define UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0AVAIL_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0AVAIL_MASK) >> 0) //!< Get extInt0Avail from bitmask extInt0
#define UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0TYPE_MASK 0x02           //!< Mask for field extInt0Type in bitmask extInt0
#define UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0TYPE_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0TYPE_MASK) >> 1) //!< Get extInt0Type from bitmask extInt0
#define UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0FEEDBACK_MASK 0x04      //!< Mask for field extInt0FeedBack in bitmask extInt0
#define UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0FEEDBACK_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT0_EXTINT0FEEDBACK_MASK) >> 2) //!< Get extInt0FeedBack from bitmask extInt0
...
#define UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1AVAIL_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1AVAIL_MASK) >> 0) //!< Get extInt1Avail from bitmask extInt1
#define UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1TYPE_MASK 0x02           //!< Mask for field extInt1Type in bitmask extInt1
#define UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1TYPE_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1TYPE_MASK) >> 1) //!< Get extInt1Type from bitmask extInt1
#define UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1FEEDBACK_MASK 0x04      //!< Mask for field extInt1FeedBack in bitmask extInt1
#define UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1FEEDBACK_GET(val) \
    (U)(((val)&UBX_MON_SMGR_DATA0_EXTINT1_EXTINT1FEEDBACK_MASK) >> 2) //!< Get extInt1FeedBack from bitmask extInt1
...
#define UBX_MON_TXBUF_U5_ERRORS_LIMIT_GET(val) \
    (U)(((val)&UBX_MON_TXBUF_U5_ERRORS_LIMIT_MASK) >> 0) //!< Get limit from bitmask errors
#define UBX_MON_TXBUF_U5_ERRORS_MEM_MASK 0x40            //!< Mask for field mem in bitmask errors
#define UBX_MON_TXBUF_U5_ERRORS_MEM_GET(val) \
    (U)(((val)&UBX_MON_TXBUF_U5_ERRORS_MEM_MASK) >> 6) //!< Get mem from bitmask errors
#define UBX_MON_TXBUF_U5_ERRORS_ALLOC_MASK 0x80        //!< Mask for field alloc in bitmask errors
#define UBX_MON_TXBUF_U5_ERRORS_ALLOC_GET(val) \
    (U)(((val)&UBX_MON_TXBUF_U5_ERRORS_ALLOC_MASK) >> 7) //!< Get alloc from bitmask errors
...
This message provides information on the status of the <i>AssistNow Autonomous</i> subsystem on the receiver. For
example, a host application can determine the optimal time to shut down the receiver by monitoring the <c>status</c>
field for a steady 0. See the chapter <r href=\"AOP-DESC'>AssistNow Autonomous</r> in the receiver description for
...
#define UBX_NAV_AOPSTATUS_DATA1_AOPCFG_USEAOP_GET(val) \
    (U)(((val)&UBX_NAV_AOPSTATUS_DATA1_AOPCFG_USEAOP_MASK) >> 0) //!< Get useAOP from bitmask aopCfg
...
This message outputs the DGPS correction data that has been applied to the current NAV Solution. See also the notes on
the <r href=\"RTCM-PROT'>RTCM protocol</r>.
...
#define UBX_NAV_DGPS_DATA0_SVID_FLAGS_CHANNEL_GET(val) \
    (U)(((val)&UBX_NAV_DGPS_DATA0_SVID_FLAGS_CHANNEL_MASK) >> 0) //!< Get channel from bitmask flags
#define UBX_NAV_DGPS_DATA0_SVID_FLAGS_DGPSUSED_MASK 0x10         //!< Mask for field dgpsUsed in bitmask flags
#define UBX_NAV_DGPS_DATA0_SVID_FLAGS_DGPSUSED_GET(val) \
    (U)(((val)&UBX_NAV_DGPS_DATA0_SVID_FLAGS_DGPSUSED_MASK) >> 4) //!< Get dgpsUsed from bitmask flags
...
/*!
Odometer Solution
This message outputs the traveled distance since last reset (see <rc href=\"UBX-NAV-RESETODO'>NAV-RESETODO</rc>)
together with an associated estimated accuracy and the total cumulated ground distance (can only be reset by a cold
start of the receiver).
...
#define UBX_NAV_ORB_DATA0_GNSSID_SVFLAG_HEALTH_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_SVFLAG_HEALTH_MASK) >> 0) //!< Get health from bitmask svFlag
#define UBX_NAV_ORB_DATA0_GNSSID_SVFLAG_VISIBILITY_MASK 0x0C      //!< Mask for field visibility in bitmask svFlag
#define UBX_NAV_ORB_DATA0_GNSSID_SVFLAG_VISIBILITY_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_SVFLAG_VISIBILITY_MASK) >> 2) //!< Get visibility from bitmask svFlag
...
#define UBX_NAV_ORB_DATA0_GNSSID_EPH_EPHUSABILITY_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_EPH_EPHUSABILITY_MASK) >> 0) //!< Get ephUsability from bitmask eph
#define UBX_NAV_ORB_DATA0_GNSSID_EPH_EPHSOURCE_MASK 0xE0             //!< Mask for field ephSource in bitmask eph
#define UBX_NAV_ORB_DATA0_GNSSID_EPH_EPHSOURCE_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_EPH_EPHSOURCE_MASK) >> 5) //!< Get ephSource from bitmask eph
...
#define UBX_NAV_ORB_DATA0_GNSSID_ALM_ALMUSABILITY_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_ALM_ALMUSABILITY_MASK) >> 0) //!< Get almUsability from bitmask alm
#define UBX_NAV_ORB_DATA0_GNSSID_ALM_ALMSOURCE_MASK 0xE0             //!< Mask for field almSource in bitmask alm
#define UBX_NAV_ORB_DATA0_GNSSID_ALM_ALMSOURCE_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_ALM_ALMSOURCE_MASK) >> 5) //!< Get almSource from bitmask alm

//@}
//! \name Bit Definitions for #UBX_NAV_ORB_DATA0_GNSSID_s::otherOrb
//@{
#define UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_ANOAOPUSABILITY_MASK \
    0x1F //!< Mask for field anoAopUsability in bitmask otherOrb
#define UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_ANOAOPUSABILITY_GET(val)        \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_ANOAOPUSABILITY_MASK) >> \
        0)                                               //!< Get anoAopUsability from bitmask otherOrb
#define UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_TYPE_MASK 0xE0 //!< Mask for field type in bitmask otherOrb
#define UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_TYPE_GET(val) \
    (U)(((val)&UBX_NAV_ORB_DATA0_GNSSID_OTHERORB_TYPE_MASK) >> 5) //!< Get type from bitmask otherOrb
...
-
   ...
   \note: See important comments concerning validity of position given in section <rb
href=\"CFG-NAV5-DESC-OUTF'>Navigation Output Filters</rb>.
...
This message outputs the Geodetic position in the currently selected ellipsoid. The default is the WGS84 Ellipsoid, but
can be changed with the message <rc href=\"UBX-CFG-DAT'>CFG-DAT</rc>.
...
This message combines position, velocity and time solution, including accuracy figures
   ...
   \note: Note that during a leap second there may be more (or less) than 60 seconds in a minute; see the <rb
href=\"LEAP-SECS-DESC'>description of leap seconds</rb> for details.
...
#define UBX_NAV_PVT_DATA1_VALID_VALIDDATE_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_VALID_VALIDDATE_MASK) >> 0) //!< Get validDate from bitmask valid
#define UBX_NAV_PVT_DATA1_VALID_VALIDTIME_MASK 0x02          //!< Mask for field validTime in bitmask valid
#define UBX_NAV_PVT_DATA1_VALID_VALIDTIME_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_VALID_VALIDTIME_MASK) >> 1) //!< Get validTime from bitmask valid
#define UBX_NAV_PVT_DATA1_VALID_FULLYRESOLVED_MASK 0x04      //!< Mask for field fullyResolved in bitmask valid
#define UBX_NAV_PVT_DATA1_VALID_FULLYRESOLVED_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_VALID_FULLYRESOLVED_MASK) >> 2) //!< Get fullyResolved from bitmask valid
...
#define UBX_NAV_PVT_DATA1_FLAGS_GNSSFIXOK_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_FLAGS_GNSSFIXOK_MASK) >> 0) //!< Get gnssFixOK from bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_DIFFSOLN_MASK 0x02           //!< Mask for field diffSoln in bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_DIFFSOLN_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_FLAGS_DIFFSOLN_MASK) >> 1) //!< Get diffSoln from bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_PSMSTATE_MASK 0x1C          //!< Mask for field psmState in bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_PSMSTATE_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_FLAGS_PSMSTATE_MASK) >> 2) //!< Get psmState from bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_HEADVEHVALID_MASK 0x20      //!< Mask for field headVehValid in bitmask flags
#define UBX_NAV_PVT_DATA1_FLAGS_HEADVEHVALID_GET(val) \
    (U)(((val)&UBX_NAV_PVT_DATA1_FLAGS_HEADVEHVALID_MASK) >> 5) //!< Get headVehValid from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_QUALITYIND_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_QUALITYIND_MASK) >> 0) //!< Get qualityInd from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_DIFFCORR_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_DIFFCORR_MASK) >> 6) //!< Get diffCorr from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_SMOOTHED_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_SMOOTHED_MASK) >> 7) //!< Get smoothed from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ORBITSOURCE_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ORBITSOURCE_MASK) >> 8) //!< Get orbitSource from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_EPHAVAIL_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_EPHAVAIL_MASK) >> 11) //!< Get ephAvail from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ALMAVAIL_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ALMAVAIL_MASK) >> 12) //!< Get almAvail from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ANOAVAIL_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_ANOAVAIL_MASK) >> 13) //!< Get anoAvail from bitmask flags
...
#define UBX_NAV_SAT_DATA0_GNSSID_FLAGS_AOPAVAIL_GET(val) \
    (U)(((val)&UBX_NAV_SAT_DATA0_GNSSID_FLAGS_AOPAVAIL_MASK) >> 14) //!< Get aopAvail from bitmask flags
...
#define UBX_NAV_SBAS_DATA0_SERVICE_RANGING_GET(val) \
    (U)(((val)&UBX_NAV_SBAS_DATA0_SERVICE_RANGING_MASK) >> 0) //!< Get Ranging from bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_CORRECTIONS_MASK 0x02      //!< Mask for field Corrections in bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_CORRECTIONS_GET(val) \
    (U)(((val)&UBX_NAV_SBAS_DATA0_SERVICE_CORRECTIONS_MASK) >> 1) //!< Get Corrections from bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_INTEGRITY_MASK 0x04            //!< Mask for field Integrity in bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_INTEGRITY_GET(val) \
    (U)(((val)&UBX_NAV_SBAS_DATA0_SERVICE_INTEGRITY_MASK) >> 2) //!< Get Integrity from bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_TESTMODE_MASK 0x08           //!< Mask for field Testmode in bitmask service
#define UBX_NAV_SBAS_DATA0_SERVICE_TESTMODE_GET(val) \
    (U)(((val)&UBX_NAV_SBAS_DATA0_SERVICE_TESTMODE_MASK) >> 3) //!< Get Testmode from bitmask service
...
This message has only been retained for backwards compatibility; users are recommended to use the <rc
href=\"UBX-NAV-PVT'>UBX-NAV-PVT</rc> message in preference.
...
#define UBX_NAV_SOL_DATA0_FLAGS_GPSFIXOK_GET(val) \
    (U)(((val)&UBX_NAV_SOL_DATA0_FLAGS_GPSFIXOK_MASK) >> 0) //!< Get GPSfixOK from bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_DIFFSOLN_MASK 0x02          //!< Mask for field DiffSoln in bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_DIFFSOLN_GET(val) \
    (U)(((val)&UBX_NAV_SOL_DATA0_FLAGS_DIFFSOLN_MASK) >> 1) //!< Get DiffSoln from bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_WKNSET_MASK 0x04            //!< Mask for field WKNSET in bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_WKNSET_GET(val) \
    (U)(((val)&UBX_NAV_SOL_DATA0_FLAGS_WKNSET_MASK) >> 2) //!< Get WKNSET from bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_TOWSET_MASK 0x08          //!< Mask for field TOWSET in bitmask flags
#define UBX_NAV_SOL_DATA0_FLAGS_TOWSET_GET(val) \
    (U)(((val)&UBX_NAV_SOL_DATA0_FLAGS_TOWSET_MASK) >> 3) //!< Get TOWSET from bitmask flags
...
-
   ...
   \note: See important comments concerning validity of position and velocity given in section <rb
href=\"CFG-NAV5-DESC-OUTF'>Navigation Output Filters</rb>.
...
typedef struct UBX_NAV_STATUS_DATA1_s {
    U4 iTOW;    //!< GPS time of week of the <r href=\"NAV-EPOCH-DESC'>navigation epoch</r>.
    U1 gpsFix;  //!< GPSfix Type, this value does <b>not</b> qualify a fix as valid and within the limits. See note on
                //! flag gpsFixOk below.
...
#define UBX_NAV_STATUS_DATA1_FLAGS_GPSFIXOK_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FLAGS_GPSFIXOK_MASK) >> 0) //!< Get gpsFixOk from bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_DIFFSOLN_MASK 0x02          //!< Mask for field diffSoln in bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_DIFFSOLN_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FLAGS_DIFFSOLN_MASK) >> 1) //!< Get diffSoln from bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_WKNSET_MASK 0x04            //!< Mask for field wknSet in bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_WKNSET_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FLAGS_WKNSET_MASK) >> 2) //!< Get wknSet from bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_TOWSET_MASK 0x08          //!< Mask for field towSet in bitmask flags
#define UBX_NAV_STATUS_DATA1_FLAGS_TOWSET_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FLAGS_TOWSET_MASK) >> 3) //!< Get towSet from bitmask flags
...
#define UBX_NAV_STATUS_DATA1_FIXSTAT_DGPSISTAT_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FIXSTAT_DGPSISTAT_MASK) >> 0) //!< Get dgpsIStat from bitmask fixStat
#define UBX_NAV_STATUS_DATA1_FIXSTAT_MAPMATCHING_MASK 0xC0        //!< Mask for field mapMatching in bitmask fixStat
#define UBX_NAV_STATUS_DATA1_FIXSTAT_MAPMATCHING_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FIXSTAT_MAPMATCHING_MASK) >> 6) //!< Get mapMatching from bitmask fixStat
...
#define UBX_NAV_STATUS_DATA1_FLAGS2_PSMSTATE_GET(val) \
    (U)(((val)&UBX_NAV_STATUS_DATA1_FLAGS2_PSMSTATE_MASK) >> 0) //!< Get psmState from bitmask flags2
...
This message has only been retained for backwards compatibility; users are recommended to use the <rc
href=\"UBX-NAV-SAT'>UBX-NAV-SAT</rc> message in preference.
...
#define UBX_NAV_SVINFO_DATA0_GLOBALFLAGS_CHIPGEN_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_GLOBALFLAGS_CHIPGEN_MASK) >> 0) //!< Get chipGen from bitmask globalFlags
...
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_SVUSED_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_SVUSED_MASK) >> 0) //!< Get svUsed from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_DIFFCORR_MASK 0x02        //!< Mask for field diffCorr in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_DIFFCORR_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_DIFFCORR_MASK) >> 1) //!< Get diffCorr from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAVAIL_MASK 0x04        //!< Mask for field orbitAvail in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAVAIL_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAVAIL_MASK) >> 2) //!< Get orbitAvail from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITEPH_MASK 0x08            //!< Mask for field orbitEph in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITEPH_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITEPH_MASK) >> 3) //!< Get orbitEph from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_UNHEALTHY_MASK 0x10         //!< Mask for field unhealthy in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_UNHEALTHY_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_UNHEALTHY_MASK) >> 4) //!< Get unhealthy from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITALM_MASK 0x20           //!< Mask for field orbitAlm in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITALM_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITALM_MASK) >> 5) //!< Get orbitAlm from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAOP_MASK 0x40          //!< Mask for field orbitAop in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAOP_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_ORBITAOP_MASK) >> 6) //!< Get orbitAop from bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_SMOOTHED_MASK 0x80          //!< Mask for field smoothed in bitmask flags
#define UBX_NAV_SVINFO_DATA0_CHN_FLAGS_SMOOTHED_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_FLAGS_SMOOTHED_MASK) >> 7) //!< Get smoothed from bitmask flags
...
#define UBX_NAV_SVINFO_DATA0_CHN_QUALITY_QUALITYIND_GET(val) \
    (U)(((val)&UBX_NAV_SVINFO_DATA0_CHN_QUALITY_QUALITYIND_MASK) >> 0) //!< Get qualityInd from bitmask quality
...
/*!
BDS Time Solution
This message reports the precise BDS time of the most recent navigation solution including validity flags and an
accuracy estimate.
...
#define UBX_NAV_TIMEBDS_DATA0_VALID_SOWVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEBDS_DATA0_VALID_SOWVALID_MASK) >> 0) //!< Get sowValid from bitmask valid
#define UBX_NAV_TIMEBDS_DATA0_VALID_WEEKVALID_MASK 0x02         //!< Mask for field weekValid in bitmask valid
#define UBX_NAV_TIMEBDS_DATA0_VALID_WEEKVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEBDS_DATA0_VALID_WEEKVALID_MASK) >> 1) //!< Get weekValid from bitmask valid
#define UBX_NAV_TIMEBDS_DATA0_VALID_LEAPSVALID_MASK 0x04         //!< Mask for field leapSValid in bitmask valid
#define UBX_NAV_TIMEBDS_DATA0_VALID_LEAPSVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEBDS_DATA0_VALID_LEAPSVALID_MASK) >> 2) //!< Get leapSValid from bitmask valid
...
/*!
GLO Time Solution
This message reports the precise GLO time of the most recent navigation solution including validity flags and an
accuracy estimate.
...
typedef struct UBX_NAV_TIMEGLO_DATA0_s {
    ...
    U2 Nt;   //!< Current date (range: 1-1461), starting at 1 from the 1st Jan of the year indicated by N4 and ending at
             //! 1461 at the 31st Dec of the third year after that indicated by N4
...
#define UBX_NAV_TIMEGLO_DATA0_VALID_TODVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEGLO_DATA0_VALID_TODVALID_MASK) >> 0) //!< Get todValid from bitmask valid
#define UBX_NAV_TIMEGLO_DATA0_VALID_DATEVALID_MASK 0x02         //!< Mask for field dateValid in bitmask valid
#define UBX_NAV_TIMEGLO_DATA0_VALID_DATEVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEGLO_DATA0_VALID_DATEVALID_MASK) >> 1) //!< Get dateValid from bitmask valid
...
/*!
GPS Time Solution
This message reports the precise GPS time of the most recent navigation solution including validity flags and an
accuracy estimate.
...
#define UBX_NAV_TIMEGPS_DATA0_VALID_TOWVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEGPS_DATA0_VALID_TOWVALID_MASK) >> 0) //!< Get towValid from bitmask valid
#define UBX_NAV_TIMEGPS_DATA0_VALID_WEEKVALID_MASK 0x02         //!< Mask for field weekValid in bitmask valid
#define UBX_NAV_TIMEGPS_DATA0_VALID_WEEKVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEGPS_DATA0_VALID_WEEKVALID_MASK) >> 1) //!< Get weekValid from bitmask valid
#define UBX_NAV_TIMEGPS_DATA0_VALID_LEAPSVALID_MASK 0x04         //!< Mask for field leapSValid in bitmask valid
#define UBX_NAV_TIMEGPS_DATA0_VALID_LEAPSVALID_GET(val) \
    (U)(((val)&UBX_NAV_TIMEGPS_DATA0_VALID_LEAPSVALID_MASK) >> 2) //!< Get leapSValid from bitmask valid
...
#define UBX_NAV_TIMEUTC_DATA0_VALID_VALIDTOW_GET(val) \
    (U)(((val)&UBX_NAV_TIMEUTC_DATA0_VALID_VALIDTOW_MASK) >> 0) //!< Get validTOW from bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_VALIDWKN_MASK 0x02          //!< Mask for field validWKN in bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_VALIDWKN_GET(val) \
    (U)(((val)&UBX_NAV_TIMEUTC_DATA0_VALID_VALIDWKN_MASK) >> 1) //!< Get validWKN from bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_VALIDUTC_MASK 0x04          //!< Mask for field validUTC in bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_VALIDUTC_GET(val) \
    (U)(((val)&UBX_NAV_TIMEUTC_DATA0_VALID_VALIDUTC_MASK) >> 2) //!< Get validUTC from bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_UTCSTANDARD_MASK 0xF0       //!< Mask for field utcStandard in bitmask valid
#define UBX_NAV_TIMEUTC_DATA0_VALID_UTCSTANDARD_GET(val) \
    (U)(((val)&UBX_NAV_TIMEUTC_DATA0_VALID_UTCSTANDARD_MASK) >> 4) //!< Get utcStandard from bitmask valid
...
-
   ...
   \note: See important comments concerning validity of velocity given in section <rb
href=\"CFG-NAV5-DESC-OUTF'>Navigation Output Filters</rb>.
...
#define UBX_RXM_PMREQ_0_FLAGS_BACKUP_GET(val) \
    (U)(((val)&UBX_RXM_PMREQ_0_FLAGS_BACKUP_MASK) >> 1) //!< Get backup from bitmask flags
...
This message contains the information needed to be able to generate a <r
href=\"ftp://ftp.igs.org/pub/data/format/rinex300.pdf'>RINEX 3</r> multi-GNSS observation file.
This message contains pseudorange, Doppler, carrier phase, phase lock and signal quality information for GNSS satellites
once signals have been synchronized. This message supports all active GNSS.
...
typedef struct UBX_RXM_RAWX_DATA0_s {
    R8 rcvTow;       //!< Measurement time of week
...
#define UBX_RXM_RAWX_DATA0_RECSTAT_LEAPSEC_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_RECSTAT_LEAPSEC_MASK) >> 0) //!< Get leapSec from bitmask recStat
#define UBX_RXM_RAWX_DATA0_RECSTAT_CLKRESET_MASK 0x02         //!< Mask for field clkReset in bitmask recStat
#define UBX_RXM_RAWX_DATA0_RECSTAT_CLKRESET_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_RECSTAT_CLKRESET_MASK) >> 1) //!< Get clkReset from bitmask recStat

//@}

//! Optional Sub-Structure of #UBX_RXM_RAWX_DATA0_t
typedef struct UBX_RXM_RAWX_DATA0_PRMES_s {
    R8 prMes; //!< Pseudorange measurement [m]. GLONASS inter frequency channel delays are compensated with an internal
              //! calibration table.
    R8 cpMes; //!< Carrier phase measurement [cycles]. The carrier phase initial ambiguity is initialized using an
    //! approximate value to make the magnitude of the phase close to the pseudorange measurement. Clock resets
    //! are applied to both phase and code measurements in accordance with the RINEX specification.
    R4 doMes;     //!< Doppler measurement (positive sign for approaching satellites) [Hz]
    U1 gnssId;    //!< GNSS identifier (see <r href=\"DESC-SVNumbering-UBX'>Satellite Numbering</r> for a list of
                  //! identifiers)
    U1 svId;      //!< Satellite identifier (see <r href=\"DESC-SVNumbering-UBX'>Satellite Numbering</r>)
    U1 reserved2; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    ...
    X1 cpStdev;   //!< Estimated carrier phase measurement standard deviation (note a raw value of 0x0F indicates the
                  //! value is invalid)
...
#define UBX_RXM_RAWX_DATA0_PRMES_PRSTDEV_PRSTD_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_PRSTDEV_PRSTD_MASK) >> 0) //!< Get prStd from bitmask prStdev
...
#define UBX_RXM_RAWX_DATA0_PRMES_CPSTDEV_CPSTD_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_CPSTDEV_CPSTD_MASK) >> 0) //!< Get cpStd from bitmask cpStdev
...
#define UBX_RXM_RAWX_DATA0_PRMES_DOSTDEV_DOSTD_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_DOSTDEV_DOSTD_MASK) >> 0) //!< Get doStd from bitmask doStdev
...
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_PRVALID_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_PRVALID_MASK) >> 0) //!< Get prValid from bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_CPVALID_MASK 0x02          //!< Mask for field cpValid in bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_CPVALID_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_CPVALID_MASK) >> 1) //!< Get cpValid from bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_HALFCYC_MASK 0x04          //!< Mask for field halfCyc in bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_HALFCYC_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_HALFCYC_MASK) >> 2) //!< Get halfCyc from bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_SUBHALFCYC_MASK 0x08       //!< Mask for field subHalfCyc in bitmask trkStat
#define UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_SUBHALFCYC_GET(val) \
    (U)(((val)&UBX_RXM_RAWX_DATA0_PRMES_TRKSTAT_SUBHALFCYC_MASK) >> 3) //!< Get subHalfCyc from bitmask trkStat
...
/*!
Raw Subframe Data
This message sends the raw data received from a certain satellite system (including <r href=\"IMES-DESC'>IMES</r>). Thus
the message contains preamble, parity bits and all protocol specific overhead and is sent out when new data is received
from the transmitter.
Note that for IMES transmitters the <c>dwrd</c> can contain data of only one IMES frame but also data of multiple
frames. For example, it could contain a single 1-word long short ID frame only, in this case <c>numWords</c> is 1, but
it could also contain a 1-word long short ID frame and a 3-word long position 1 frame, then <c>numWords</c> would be 4.
For details on the data format, please check IS-QZSS Version 1.5.
...
This message has only been retained for backwards compatibility; users are recommended to use the <rc
href=\"UBX-NAV-ORB'>UBX-NAV-ORB</rc> message in preference.
...
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_URA_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_SVFLAG_URA_MASK) >> 0) //!< Get ura from bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_HEALTHY_MASK 0x10      //!< Mask for field healthy in bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_HEALTHY_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_SVFLAG_HEALTHY_MASK) >> 4) //!< Get healthy from bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_EPHVAL_MASK 0x20           //!< Mask for field ephVal in bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_EPHVAL_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_SVFLAG_EPHVAL_MASK) >> 5) //!< Get ephVal from bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_ALMVAL_MASK 0x40          //!< Mask for field almVal in bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_ALMVAL_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_SVFLAG_ALMVAL_MASK) >> 6) //!< Get almVal from bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_NOTAVAIL_MASK 0x80        //!< Mask for field notAvail in bitmask svFlag
#define UBX_RXM_SVSI_DATA0_SVID_SVFLAG_NOTAVAIL_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_SVFLAG_NOTAVAIL_MASK) >> 7) //!< Get notAvail from bitmask svFlag
...
#define UBX_RXM_SVSI_DATA0_SVID_AGE_ALMAGE_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_AGE_ALMAGE_MASK) >> 0) //!< Get almAge from bitmask age
#define UBX_RXM_SVSI_DATA0_SVID_AGE_EPHAGE_MASK 0xF0          //!< Mask for field ephAge in bitmask age
#define UBX_RXM_SVSI_DATA0_SVID_AGE_EPHAGE_GET(val) \
    (U)(((val)&UBX_RXM_SVSI_DATA0_SVID_AGE_EPHAGE_MASK) >> 4) //!< Get ephAge from bitmask age
...
The receiver sends this message when it is disciplining an external oscillator and the external oscillator is set up to
be controlled via the host.
...
typedef struct UBX_TIM_DOSC_DATA0_s {
    U1 version;      //!< Message version (0 for this version)
    U1 reserved1[3]; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U4 value; //!< The raw value to be applied to the DAC controlling the external oscillator. The least significant
              //! bits should be written to the DAC, with the higher bits being ignored.
...
This message reports frequency changes commanded by the sync manager for the internal and external oscillator. It is
output at the configured rate even if the sync manager decides not to command a frequency change.
...
typedef struct UBX_TIM_FCHG_DATA0_s {
    U1 version;      //!< Message version (0 for this version)
    U1 reserved1[3]; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    U4 iTOW; //!< GPS time of week of the <r href=\"NAV-EPOCH-DESC'>navigation epoch</r> from which the sync manager
             //! obtains the GNSS specific data.
...
This message can be sent by the host to force the receiver to bypass the disciplining algorithms in the SMGR and carry
out the instructed changes to internal or external oscillator frequency. No checks are carried out on the size of the
frequency change requested, so normal limits imposed by the SMGR are ignored.
It is recommended that the disciplining of that oscillator is disabled before this message is sent (i.e. by clearing the
enableInternal or enableExternal flag in the <rc href=\"UBX-CFG-SMGR-DATA0'>CFG-SMGR</rc> message), otherwise the
autonomous disciplining processes may cancel the effect of the direct command.
Note that the GNSS subsystem may temporarily lose track of some/all satellite signals if a large change of the internal
oscillator is made.
...
#define UBX_TIM_HOC_DATA0_FLAGS_RAW_GET(val) \
    (U)(((val)&UBX_TIM_HOC_DATA0_FLAGS_RAW_MASK) >> 0) //!< Get raw from bitmask flags
#define UBX_TIM_HOC_DATA0_FLAGS_DIFFERENCE_MASK 0x02   //!< Mask for field difference in bitmask flags
#define UBX_TIM_HOC_DATA0_FLAGS_DIFFERENCE_GET(val) \
    (U)(((val)&UBX_TIM_HOC_DATA0_FLAGS_DIFFERENCE_MASK) >> 1) //!< Get difference from bitmask flags
...
/*!
Source measurement
Frequency and/or phase measurement of syncronization sources. The measurements are relative to the nominal frequency and
nominal phase.
The receiver reports the measurements on its sync sources using this message. Which measurements are reported can be
configured using UBX-CFG-SMGR.
The host may report offset of the receiver's outputs with this message as well. The receiver has to be configured using
UBX-CFG-SMGR to enable the use of the external measurement messages. Otherwise the receiver will ignore them.
...
typedef struct UBX_TIM_SMEAS_DATA0_SOURCEID_s {
    U1 sourceId; //!< Index of source. SMEAS can provide six measurement sources. The first four sourceId values
    //! represent measurements made by the receiver and sent to the host. The first of these with a sourceId
    //! value of 0 is a measurement of the internal oscillator against the current receiver
    //! time-and-frequency estimate. The internal oscillator is being disciplined against that estimate and
    //! this result represents the current offset between the actual and desired internal oscillator states.
    //! The next three sourceId values represent frequency and time measurements made by the receiver
    //! against the internal oscillator. sourceId 1 represents the GNSS-derived frequency and time compared
    //! with the internal oscillator frequency and time. sourceId2 give measurements of a signal coming in
    //! on EXTINT0. sourceId 3 corresponds to a similar measurement on EXTINT1. The remaining two of these
    //! measurements (sourceId 4 and 5) are made by the host and sent to the receiver. A measurement with
    //! sourceId 4 is a measurement by the host of the internal oscillator and sourceId 5 indicates a host
    ...
    U1 reserved3[4]; //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    I4 freqOffset; //!< Frequency offset, positive if the source frequency is too high, negative if the frequency is too
                   //! low.
...
#define UBX_TIM_SMEAS_DATA0_SOURCEID_FLAGS_FREQVALID_GET(val) \
    (U)(((val)&UBX_TIM_SMEAS_DATA0_SOURCEID_FLAGS_FREQVALID_MASK) >> 0) //!< Get freqValid from bitmask flags
#define UBX_TIM_SMEAS_DATA0_SOURCEID_FLAGS_PHASEVALID_MASK 0x02         //!< Mask for field phaseValid in bitmask flags
#define UBX_TIM_SMEAS_DATA0_SOURCEID_FLAGS_PHASEVALID_GET(val) \
    (U)(((val)&UBX_TIM_SMEAS_DATA0_SOURCEID_FLAGS_PHASEVALID_MASK) >> 1) //!< Get phaseValid from bitmask flags
...
/*!
Survey-in data
This message contains information about survey-in parameters. For details about the Time Mode see section <r
href=\"CFG-TMODE-DESC'>Time Mode Configuration</r>.
...
The delay figures and timebase given in <rc href=\"UBX-CFG-TP5'>CFG-TP5</rc> are also applied to the time results output
in this message.
...
#define UBX_TIM_TM2_DATA1_FLAGS_MODE_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_MODE_MASK) >> 0) //!< Get mode from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_RUN_MASK 0x02           //!< Mask for field run in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_RUN_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_RUN_MASK) >> 1)   //!< Get run from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_NEWFALLINGEDGE_MASK 0x04 //!< Mask for field newFallingEdge in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_NEWFALLINGEDGE_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_NEWFALLINGEDGE_MASK) >> 2) //!< Get newFallingEdge from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_TIMEBASE_MASK 0x18                //!< Mask for field timeBase in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_TIMEBASE_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_TIMEBASE_MASK) >> 3) //!< Get timeBase from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_UTC_MASK 0x20               //!< Mask for field utc in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_UTC_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_UTC_MASK) >> 5) //!< Get utc from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_TIME_MASK 0x40         //!< Mask for field time in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_TIME_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_TIME_MASK) >> 6) //!< Get time from bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_NEWRISINGEDGE_MASK 0x80 //!< Mask for field newRisingEdge in bitmask flags
#define UBX_TIM_TM2_DATA1_FLAGS_NEWRISINGEDGE_GET(val) \
    (U)(((val)&UBX_TIM_TM2_DATA1_FLAGS_NEWRISINGEDGE_MASK) >> 7) //!< Get newRisingEdge from bitmask flags
...
This message contains information about the time pulse that has just happened and the state of the disciplined
oscillators(s) at the time of the pulse. It gives the UTC and GNSS times and time uncertainty of the pulse together with
frequency and frequency uncertainty of the disciplined oscillators. It also supplies leap second information.
...
typedef struct UBX_TIM_TOS_DATA0_s {
    U1 version;           //!< Message version (0 for this version)
    U1 gnssId;            //!< GNSS system used for reporting GNSS time (see <r href=\"DESC-SVNumbering-UBX'>Satellite
                          //! Numbering</r>)
    U1 reserved1[2];      //!< <r href='DESC-UBX-Reserved'>Reserved</r>
    X4 flags;             //!< Flags
...
#define UBX_TIM_TOS_DATA0_FLAGS_LEAPNOW_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_LEAPNOW_MASK) >> 0) //!< Get leapNow from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LEAPSOON_MASK 0x02         //!< Mask for field leapSoon in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LEAPSOON_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_LEAPSOON_MASK) >> 1) //!< Get leapSoon from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LEAPPOSITIVE_MASK 0x04      //!< Mask for field leapPositive in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LEAPPOSITIVE_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_LEAPPOSITIVE_MASK) >> 2) //!< Get leapPositive from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_TIMEINLIMIT_MASK 0x08           //!< Mask for field timeInLimit in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_TIMEINLIMIT_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_TIMEINLIMIT_MASK) >> 3) //!< Get timeInLimit from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_INTOSCINLIMIT_MASK 0x10        //!< Mask for field intOscInLimit in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_INTOSCINLIMIT_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_INTOSCINLIMIT_MASK) >> 4) //!< Get intOscInLimit from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_EXTOSCINLIMIT_MASK 0x20          //!< Mask for field extOscInLimit in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_EXTOSCINLIMIT_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_EXTOSCINLIMIT_MASK) >> 5) //!< Get extOscInLimit from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_GNSSTIMEVALID_MASK 0x40          //!< Mask for field gnssTimeValid in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_GNSSTIMEVALID_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_GNSSTIMEVALID_MASK) >> 6) //!< Get gnssTimeValid from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_UTCTIMEVALID_MASK 0x80           //!< Mask for field UTCTimeValid in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_UTCTIMEVALID_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_UTCTIMEVALID_MASK) >> 7) //!< Get UTCTimeValid from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_DISCSRC_MASK 0x700              //!< Mask for field DiscSrc in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_DISCSRC_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_DISCSRC_MASK) >> 8) //!< Get DiscSrc from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_RAIM_MASK 0x800            //!< Mask for field raim in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_RAIM_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_RAIM_MASK) >> 11) //!< Get raim from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_COHPULSE_MASK 0x1000     //!< Mask for field cohPulse in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_COHPULSE_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_COHPULSE_MASK) >> 12) //!< Get cohPulse from bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LOCKEDPULSE_MASK 0x2000      //!< Mask for field lockedPulse in bitmask flags
#define UBX_TIM_TOS_DATA0_FLAGS_LOCKEDPULSE_GET(val) \
    (U)(((val)&UBX_TIM_TOS_DATA0_FLAGS_LOCKEDPULSE_MASK) >> 13) //!< Get lockedPulse from bitmask flags
...
/*!
Time Pulse Timedata
This message contains information for high precision timing. The recommended configuration when using this message is to
set both the measurement rate (<rc href=\"UBX-CFG-RATE'>CFG-RATE</rc>) and the timepulse frequency (<rc
href=\"UBX-CFG-TP5'>CFG-TP5</rc>) to 1Hz. For more information see section <r href=\"CFG-TP5-DESC'>Time pulse</r>.
...
#define UBX_TIM_TP_DATA0_FLAGS_TIMEBASE_GET(val) \
    (U)(((val)&UBX_TIM_TP_DATA0_FLAGS_TIMEBASE_MASK) >> 0) //!< Get timeBase from bitmask flags
#define UBX_TIM_TP_DATA0_FLAGS_UTC_MASK 0x02               //!< Mask for field utc in bitmask flags
#define UBX_TIM_TP_DATA0_FLAGS_UTC_GET(val) \
    (U)(((val)&UBX_TIM_TP_DATA0_FLAGS_UTC_MASK) >> 1) //!< Get utc from bitmask flags
#define UBX_TIM_TP_DATA0_FLAGS_RAIM_MASK 0x0C         //!< Mask for field raim in bitmask flags
#define UBX_TIM_TP_DATA0_FLAGS_RAIM_GET(val) \
    (U)(((val)&UBX_TIM_TP_DATA0_FLAGS_RAIM_MASK) >> 2) //!< Get raim from bitmask flags
...
#define UBX_TIM_TP_DATA0_REFINFO_TIMEREFGNSS_GET(val) \
    (U)(((val)&UBX_TIM_TP_DATA0_REFINFO_TIMEREFGNSS_MASK) >> 0) //!< Get timeRefGnss from bitmask refInfo
#define UBX_TIM_TP_DATA0_REFINFO_UTCSTANDARD_MASK 0xF0          //!< Mask for field utcStandard in bitmask refInfo
#define UBX_TIM_TP_DATA0_REFINFO_UTCSTANDARD_GET(val) \
    (U)(((val)&UBX_TIM_TP_DATA0_REFINFO_UTCSTANDARD_MASK) >> 4) //!< Get utcStandard from bitmask refInfo
...
Calibrate (measure) gain of the voltage controlled oscillator. The calibration is performed by varying the raw
oscillator control values between the limits specified in raw0 amd raw1. maxStepSize is the largest step change that can
be used during the calibration process. The \"raw values\" are either PWM duty cycle values or DAC values depending on
how the VCTCXO is connected to the system. The measured gain is the transfer function dRelativeFrequencyChange/dRaw (not
dFrequency/dVoltage). The calibration process works as follows:
Starting from the current raw output the control value is changed in the direction of raw0 in steps of size at most
maxStepSize. Then the frequency is measured and the control value is changed towards raw1, again in steps of
maxStepSize. When raw1 is reached, the frequency is again measured and the message version DATA0 is output containing
the measured result. Normal operation then resumes. If the control value movement is less than maxStepSize then the
transition will happen in one step - this will give fast calibration.
Care must be taken when calibrating the internal oscillator against the GNSS source. In that case the changes applied to
the oscillator frequency could be severe enough to lose satellite signal tracking, especially when signals are weak. If
too many signals are lost, the GNSS system will lose its fix and be unable to measure the oscillator frequency - the
calibration will then fail. In this case maxStepSize must be reasonably small.
It is also important that only the chosen frequency source is enabled during the calibration process and that it remains
stable throughout the calibration period; otherwise incorrect oscillator measurements will be made and this will lead to
miscalibration and poor subsequent operation of the receiver.
...
This message is sent when the oscillator gain calibration process is finished (successful or unsuccessful). It notifies
the user of the calibrated oscillator gain. If the oscillator gain calibration process was successful, this message will
contain the measured gain (field gainVco) and its uncertainty (field gainUncertainty). The calibration process can
however fail. In that case the two fields gainVco and gainUncertainty are set to zero.
...
#define UBX_TIM_VRFY_DATA1_FLAGS_SRC_GET(val) \
    (U)(((val)&UBX_TIM_VRFY_DATA1_FLAGS_SRC_MASK) >> 0) //!< Get src from bitmask flags
...
Sending this (empty / no-payload) message to the receiver results in the receiver returning a <i>System Restored from
Backup</i> message as defined below.
...
The host can send this message in order to save part of the BBR memory in a file in flash file system. The feature is
designed in order to emulate the presence of the backup battery even if it is not present; the host can issue the save
on shutdown command before switching off the device supply. It is recommended to issue a GNSS stop command before, in
order to keep the BBR memory content consistent.
...
The host can send this message in order to erase the backup file present in flash. It is recommended that the clear
operation is issued after the host has received the notification that the memory has been restored after a reset.
Alternatively the host can parse the startup string 'Restored data saved on shutdown' or poll the UBX-UPD-SOS message
...
The message is sent from the device as confirmation of creation of a backup file in flash. The host can safely shut down
the device after received this message.
...
The message is sent from the device to notify the host the BBR has been restored from a backup file in flash. The host
should clear the backup file after receiving this message. If the UBX-UPD-SOS message is polled, this message will be
resent.
